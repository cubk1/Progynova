...
public class 宇轩的头部显示 extends 宇轩の模块 {
    ...
    public void 宇轩渲染事件(宇轩の渲染2D事件 事件) {
        ...
        switch (宇轩美照颜色模式.获取设置()){
            ...
            case "彩虹":{
                Color rainbow = new Color(Color.HSBtoRGB((float)((double)this.我D世界.宇轩游玩者.ticksExisted / 50.0 + Math.sin((double)0 / 50.0 * 1.6)) % 1.0f, 1f, 1.0f));
                颜色 = rainbow.getRGB();
            }
        }
        ...
        if(DEOBF.获取设置()){
            ...
            渲染的功能.渲染图片(new ResourceLocation("liyuxuan1337/DEOBF.png"),0,0,32,32,颜色);
            y += 32;
        }
        ...
        if(瞎几把转.获取设置()){
            ...
            while(i < 100){
                ...
                ScaledResolution 宇轩的位置 = new ScaledResolution(我D世界);
                渲染的功能.渲染图片(new ResourceLocation("liyuxuan1337/DEOBF.png"),宇轩的随机数.nextInt(宇轩的位置.getScaledWidth()),宇轩的随机数.nextInt(宇轩的位置.getScaledHeight()),60,50, 颜色);
                i++;
            }
        }

        我D世界.fontRendererObj.drawStringWithShadow("L" + EnumChatFormatting.WHITE + "iYuXuan1337 - 想学写热注入的可爱的女孩子~",2, y, new Color(246,168,192).getRGB());
        y += 我D世界.fontRendererObj.FONT_HEIGHT + 2;
        ArrayList<宇轩の模块> 宇轩の模块们但是排列过 = new ArrayList<>(宇轩科技.获取李宇轩1337().获取宇轩的模块管理员().获取宇轩的模块们());
        宇轩の模块们但是排列过.sort((o1, o2) -> 我D世界.fontRendererObj.getStringWidth(o2.获取名字()) - 我D世界.fontRendererObj.getStringWidth(o1.获取名字()));

        for(宇轩の模块 模块 : 宇轩の模块们但是排列过){
            if(!模块.是不是开启的())
                continue;
            我D世界.fontRendererObj.drawStringWithShadow(模块.获取名字(),2,y,模块.获取宇轩的模块种类().获取宇轩颜色().getRGB());
            y += 我D世界.fontRendererObj.FONT_HEIGHT + 2;
        }
    }
