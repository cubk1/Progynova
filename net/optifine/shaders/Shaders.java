...
public class Shaders
{
    ...
    private static void bindCustomTextures(ICustomTexture[] cts)
    {
        if (cts != null)
        {
            for (int i = 0; i < cts.length; ++i)
            {
                ...
                光照状态经理.setActiveTexture(33984 + icustomtexture.getTextureUnit());
    ...
    private static void saveOptionProperties(IShaderPack sp, Properties props) throws IOException
    {
        ...
        File file1 = new File(我的手艺.得到我的手艺().mcDataDir, s);
    ...
    {
        mc.ingameGUI.getChatGUI().printChatMessage(new 交流组分文本(str));
    }
    ...
    public static void startup(我的手艺 mc)
    {
    ...
    public static void useProgram(Program program)
    {
        ...
        if (activeProgram != program)
        {
            ...
            if (i != 0)
            {
                ...
                ItemStack itemstack = mc.宇轩游玩者 != null ? mc.宇轩游玩者.getHeldItem() : null;
                ...
                setProgramUniform1f(uniform_far, (float)(mc.游戏一窝.renderDistanceChunks * 16));
                ...
                setProgramUniform1f(uniform_screenBrightness, mc.游戏一窝.gammaSetting);
                setProgramUniform1i(uniform_hideGUI, mc.游戏一窝.hideGUI ? 1 : 0);
    ...
    public static void uninit()
    {
        if (isShaderPackInitialized)
        {
            ...
            {
                光照状态经理.deleteTextures(dfbDepthTextures);
            ...
            {
                光照状态经理.deleteTextures(dfbColorTextures);
            ...
            {
                光照状态经理.deleteTextures(sfbDepthTextures);
            ...
            {
                光照状态经理.deleteTextures(sfbColorTextures);
    ...
    private static void setupFrameBuffer()
    {
        ...
        {
            光照状态经理.绑定手感(dfbDepthTextures.get(i));
        ...
        {
            光照状态经理.绑定手感(dfbColorTexturesFlip.getA(k));
            GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_S, GL12.GL_CLAMP_TO_EDGE);
            GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_T, GL12.GL_CLAMP_TO_EDGE);
        ...
        {
            光照状态经理.绑定手感(dfbColorTexturesFlip.getB(l));
            GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_S, GL12.GL_CLAMP_TO_EDGE);
            GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_T, GL12.GL_CLAMP_TO_EDGE);
            GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, GL11.GL_LINEAR);
            GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, GL11.GL_LINEAR);
        ...
        if (i1 == 36058)
        {
            ...
            {
                光照状态经理.绑定手感(dfbColorTexturesFlip.getA(j));
    ...
    private static void setupShadowFrameBuffer()
    {
        if (usedShadowDepthBuffers != 0)
        {
            ...
            {
                光照状态经理.绑定手感(sfbDepthTextures.get(i));
            ...
            {
                光照状态经理.绑定手感(sfbColorTextures.get(k));
                GL11.glTexParameterf(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_S, 33071.0F);
                GL11.glTexParameterf(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_T, 33071.0F);
    ...
    public static void beginRender(我的手艺 宇轩的世界, float partialTicks, long finishTimeNano)
    {
        checkGLError("pre beginRender");
        checkWorldChanged(mc.宇轩の世界);
        mc = 宇轩的世界;
        ...
        worldTime = mc.宇轩の世界.getWorldTime();
        ...
        moonPhase = mc.宇轩の世界.getMoonPhase();
        ++frameCounter;
        ...
        rainStrength = 宇轩的世界.宇轩の世界.getRainStrength(partialTicks);
        ...
        if (entity != null)
        {
            ...
            if (mc.宇轩游玩者 != null)
            {
                ...
                if (mc.宇轩游玩者.isPotionActive(Potion.nightVision))
                {
                    nightVision = Config.getMinecraft().entityRenderer.getNightVisionBrightness(mc.宇轩游玩者, partialTicks);
                }

                blindness = 0.0F;

                if (mc.宇轩游玩者.isPotionActive(Potion.blindness))
                {
                    int i = mc.宇轩游玩者.getActivePotionEffect(Potion.blindness).getDuration();
    ...
    private static void bindGbuffersTextures()
    {
        if (usedShadowDepthBuffers >= 1)
        {
            ...
            光照状态经理.绑定手感(sfbDepthTextures.get(0));

            if (usedShadowDepthBuffers >= 2)
            {
                光照状态经理.setActiveTexture(33989);
                光照状态经理.绑定手感(sfbDepthTextures.get(1));
            }
        }
        ...
        {
            光照状态经理.绑定手感(dfbColorTexturesFlip.getA(i));
            GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, GL11.GL_LINEAR);
            GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, GL11.GL_LINEAR);
            光照状态经理.绑定手感(dfbColorTexturesFlip.getB(i));
            GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, GL11.GL_LINEAR);
            GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, GL11.GL_LINEAR);
        }
        ...
        {
            光照状态经理.setActiveTexture(33991 + j);
            光照状态经理.绑定手感(dfbColorTexturesFlip.getA(4 + j));
        }

        光照状态经理.setActiveTexture(33990);
        光照状态经理.绑定手感(dfbDepthTextures.get(0));

        if (usedDepthBuffers >= 2)
        {
            光照状态经理.setActiveTexture(33995);
            光照状态经理.绑定手感(dfbDepthTextures.get(1));

            if (usedDepthBuffers >= 3)
            {
                光照状态经理.setActiveTexture(33996);
                光照状态经理.绑定手感(dfbDepthTextures.get(2));
            }
        }

        for (int k = 0; k < usedShadowColorBuffers; ++k)
        {
            光照状态经理.setActiveTexture(33997 + k);
            光照状态经理.绑定手感(sfbColorTextures.get(k));
        }

        if (noiseTextureEnabled)
        {
            光照状态经理.setActiveTexture(33984 + noiseTexture.getTextureUnit());
            光照状态经理.绑定手感(noiseTexture.getTextureId());
        }
    ...
    {
        光照状态经理.clearColor(red, green, blue, alpha);
        clearColorR = red;
        clearColorG = green;
        clearColorB = blue;
    }
    ...
    public static void setCameraShadow(float partialTicks)
    {
        ...
        celestialAngle = mc.宇轩の世界.getCelestialAngle(partialTicks);
    ...
    public static void genCompositeMipmap()
    {
        if (hasGlGenMipmap)
        {
            for (int i = 0; i < usedColorBuffers; ++i)
            {
                ...
                {
                    光照状态经理.setActiveTexture(33984 + colorTextureImageUnit[i]);
    ...
    public static void renderDeferred()
    {
        if (!isShadowPass)
        {
            ...
            if (flag)
            {
                ...
                光照状态经理.setActiveTexture(33984);
                mc.得到手感经理().绑定手感(TextureMap.locationBlocksTexture);
            }
        }
    }
                ...
                if (program.getId() != 0)
                {
                    ...
                    for (int j = 0; j < usedColorBuffers; ++j)
                    {
                        if (program.getToggleColorTextures()[j])
                        {
                            ...
                            光照状态经理.setActiveTexture(33984 + colorTextureImageUnit[j]);
                            光照状态经理.绑定手感(dfbColorTexturesFlip.getA(j));
    ...
    private static void renderFinal()
    {
        ...
        if (EntityRenderer.anaglyphEnable)
        {
            ...
            光照状态经理.colorMask(flag, !flag, !flag, true);
        }
    ...
    public static void drawHorizon()
    {
        ...
        float f = (float)(mc.游戏一窝.renderDistanceChunks * 16);
    ...
    static
    {
        shaderPacksDir = new File(我的手艺.得到我的手艺().mcDataDir, "shaderpacks");
        configFile = new File(我的手艺.得到我的手艺().mcDataDir, "optionsshaders.txt");
    }
}
