...
{
    private static 我的手艺 mc = Config.getMinecraft();
    ...
    public static ResourceLocation getTextureLocation(ResourceLocation loc)
    {
        ...
        {
            鬼Screen guiscreen = mc.currentScreen;

            if (!(guiscreen instanceof 鬼Container))
            {
                return loc;
            }
            else if (loc.getResourceDomain().equals("minecraft") && loc.getResourcePath().startsWith("textures/gui/"))
            {
                ...
                {
                    IBlockAccess iblockaccess = mc.宇轩の世界;

                    if (iblockaccess == null)
                    {
                        return loc;
                    }
                    else if (guiscreen instanceof 鬼ContainerCreative)
                    {
                        return getTexturePos(CustomGuiProperties.EnumContainer.CREATIVE, mc.宇轩游玩者.getPosition(), iblockaccess, loc, guiscreen);
                    }
                    else if (guiscreen instanceof 鬼Inventory)
                    {
                        return getTexturePos(CustomGuiProperties.EnumContainer.INVENTORY, mc.宇轩游玩者.getPosition(), iblockaccess, loc, guiscreen);
                    }
                    else
                    {
