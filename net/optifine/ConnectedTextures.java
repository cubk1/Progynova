...
public class ConnectedTextures
{
    ...
    public static BakedQuad[] getConnectedTexture(IBlockAccess blockAccess, IBlockState blockState, 阻止位置 阻止位置, BakedQuad quad, RenderEnv renderEnv)
    {
        ...
        else
        {
            ...
            if (skipConnectedTexture(blockAccess, blockState, 阻止位置, quad, renderEnv))
            {
            ...
            else
            {
                ...
                BakedQuad[] abakedquad = getConnectedTextureMultiPass(blockAccess, blockState, 阻止位置, enumfacing, quad, renderEnv);
                return abakedquad;
            }
        }
    }

    private static boolean skipConnectedTexture(IBlockAccess blockAccess, IBlockState blockState, 阻止位置 阻止位置, BakedQuad quad, RenderEnv renderEnv)
    {
        Block block = blockState.getBlock();
        ...
        if (block instanceof BlockPane)
        {
            ...
            {
                IBlockState iblockstate1 = blockAccess.getBlockState(阻止位置.offset(quad.getFace()));
        ...
        if (block instanceof BlockPane)
        {
            ...
            阻止位置 blockpos = 阻止位置.offset(quad.getFace());
    ...
    private static BakedQuad[] getConnectedTextureMultiPass(IBlockAccess blockAccess, IBlockState blockState, 阻止位置 阻止位置, EnumFacing side, BakedQuad quad, RenderEnv renderEnv)
    {
        BakedQuad[] abakedquad = getConnectedTextureSingle(blockAccess, blockState, 阻止位置, side, quad, true, 0, renderEnv);
        ...
        else
        {
            ...
            for (int i = 0; i < list.size(); ++i)
            {
                ...
                for (int j = 0; j < 3; ++j)
                {
                    BakedQuad[] abakedquad1 = getConnectedTextureSingle(blockAccess, blockState, 阻止位置, side, bakedquad1, false, j + 1, renderEnv);
    ...
    public static BakedQuad[] getConnectedTextureSingle(IBlockAccess blockAccess, IBlockState blockState, 阻止位置 阻止位置, EnumFacing facing, BakedQuad quad, boolean checkBlocks, int pass, RenderEnv renderEnv)
    {
        Block block = blockState.getBlock();
                ...
                if (i >= 0 && i < tileProperties.length)
                {
                    ...
                    if (aconnectedproperties != null)
                    {
                        ...
                        for (int k = 0; k < aconnectedproperties.length; ++k)
                        {
                            ...
                            {
                                BakedQuad[] abakedquad = getConnectedTexture(connectedproperties, blockAccess, blockstatebase, 阻止位置, j, quad, pass, renderEnv);
                ...
                if (l >= 0 && l < blockProperties.length)
                {
                    ...
                    if (aconnectedproperties1 != null)
                    {
                        ...
                        for (int j1 = 0; j1 < aconnectedproperties1.length; ++j1)
                        {
                            ...
                            {
                                BakedQuad[] abakedquad1 = getConnectedTexture(connectedproperties1, blockAccess, blockstatebase, 阻止位置, i1, quad, pass, renderEnv);
    ...
    private static BakedQuad[] getConnectedTexture(ConnectedProperties cp, IBlockAccess blockAccess, BlockStateBase blockState, 阻止位置 阻止位置, int side, BakedQuad quad, int pass, RenderEnv renderEnv)
    {
        int i = 0;
        ...
        else
        {
            ...
            else
            {
                if (cp.biomes != null)
                {
                    BiomeGenBase biomegenbase = blockAccess.getBiomeGenForCoords(阻止位置);
                ...
                if (cp.nbtName != null)
                {
                    String s = TileEntityUtils.getTileEntityName(blockAccess, 阻止位置);
                ...
                {
                    case 1:
                        return getQuads(getConnectedTextureCtm(cp, blockAccess, blockState, 阻止位置, i, side, textureatlassprite, j, renderEnv), quad, renderEnv);

                    case 2:
                        return getQuads(getConnectedTextureHorizontal(cp, blockAccess, blockState, 阻止位置, i, side, textureatlassprite, j), quad, renderEnv);

                    case 3:
                        return getQuads(getConnectedTextureTop(cp, blockAccess, blockState, 阻止位置, i, side, textureatlassprite, j), quad, renderEnv);

                    case 4:
                        return getQuads(getConnectedTextureRandom(cp, blockAccess, blockState, 阻止位置, side), quad, renderEnv);

                    case 5:
                        return getQuads(getConnectedTextureRepeat(cp, 阻止位置, side), quad, renderEnv);

                    case 6:
                        return getQuads(getConnectedTextureVertical(cp, blockAccess, blockState, 阻止位置, i, side, textureatlassprite, j), quad, renderEnv);

                    case 7:
                        return getQuads(getConnectedTextureFixed(cp), quad, renderEnv);

                    case 8:
                        return getQuads(getConnectedTextureHorizontalVertical(cp, blockAccess, blockState, 阻止位置, i, side, textureatlassprite, j), quad, renderEnv);

                    case 9:
                        return getQuads(getConnectedTextureVerticalHorizontal(cp, blockAccess, blockState, 阻止位置, i, side, textureatlassprite, j), quad, renderEnv);

                    case 10:
                        if (pass == 0)
                        {
                            return getConnectedTextureCtmCompact(cp, blockAccess, blockState, 阻止位置, i, side, quad, j, renderEnv);
                        }
                    ...
                    case 11:
                        return getConnectedTextureOverlay(cp, blockAccess, blockState, 阻止位置, i, side, quad, j, renderEnv);

                    case 12:
                        return getConnectedTextureOverlayFixed(cp, quad, renderEnv);

                    case 13:
                        return getConnectedTextureOverlayRandom(cp, blockAccess, blockState, 阻止位置, side, quad, renderEnv);

                    case 14:
                        return getConnectedTextureOverlayRepeat(cp, 阻止位置, side, quad, renderEnv);

                    case 15:
                        return getConnectedTextureOverlayCtm(cp, blockAccess, blockState, 阻止位置, i, side, quad, j, renderEnv);
                }
            }
        }
    }
    ...
    private static TextureAtlasSprite getConnectedTextureRandom(ConnectedProperties cp, IBlockAccess blockAccess, BlockStateBase blockState, 阻止位置 阻止位置, int side)
    {
        ...
        else
        {
            ...
            int l = Config.getRandom(阻止位置, i) & Integer.MAX_VALUE;
    ...
    private static TextureAtlasSprite getConnectedTextureRepeat(ConnectedProperties cp, 阻止位置 阻止位置, int side)
    {
        if (cp.tileIcons.length == 1)
        {
            return cp.tileIcons[0];
    ...
    private static TextureAtlasSprite getConnectedTextureCtm(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, 阻止位置 阻止位置, int vertAxis, int side, TextureAtlasSprite icon, int metadata, RenderEnv renderEnv)
    {
        int i = getConnectedTextureCtmIndex(cp, blockAccess, blockState, 阻止位置, vertAxis, side, icon, metadata, renderEnv);
        return cp.tileIcons[i];
    }

    private static synchronized BakedQuad[] getConnectedTextureCtmCompact(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, 阻止位置 阻止位置, int vertAxis, int side, BakedQuad quad, int metadata, RenderEnv renderEnv)
    {
        TextureAtlasSprite textureatlassprite = quad.getSprite();
        int i = getConnectedTextureCtmIndex(cp, blockAccess, blockState, 阻止位置, vertAxis, side, textureatlassprite, metadata, renderEnv);
        return ConnectedTexturesCompact.getConnectedTextureCtmCompact(i, cp, side, quad, renderEnv);
    }

    private static BakedQuad[] getConnectedTextureOverlay(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, 阻止位置 阻止位置, int vertAxis, int side, BakedQuad quad, int metadata, RenderEnv renderEnv)
    {
        if (!quad.isFullQuad())
        {
            return null;
        ...
        else
        {
            ...
            for (int i = 0; i < 4; ++i)
            {
                aboolean[i] = isNeighbourOverlay(cp, blockAccess, blockState, ablockdir[i].offset(阻止位置), side, textureatlassprite, metadata);
            }
            ...
            try
            {
                if (!aboolean[0] || !aboolean[1] || !aboolean[2] || !aboolean[3])
                {
                    ...
                    for (int j = 0; j < 4; ++j)
                    {
                        aboolean1[j] = isNeighbourOverlay(cp, blockAccess, blockState, ablockdir1[j].offset(阻止位置), side, textureatlassprite, metadata);
                    }
                    ...
                    for (int k = 0; k < 4; ++k)
                    {
                        aboolean2[k] = isNeighbourMatching(cp, blockAccess, blockState, ablockdir[k].offset(阻止位置), side, textureatlassprite, metadata);
                    }
    ...
    private static BakedQuad[] getConnectedTextureOverlayRandom(ConnectedProperties cp, IBlockAccess blockAccess, BlockStateBase blockState, 阻止位置 阻止位置, int side, BakedQuad quad, RenderEnv renderEnv)
    {
        if (!quad.isFullQuad())
        {
            return null;
        ...
        else
        {
            ...
            try
            {
                TextureAtlasSprite textureatlassprite = getConnectedTextureRandom(cp, blockAccess, blockState, 阻止位置, side);
    ...
    private static BakedQuad[] getConnectedTextureOverlayRepeat(ConnectedProperties cp, 阻止位置 阻止位置, int side, BakedQuad quad, RenderEnv renderEnv)
    {
        if (!quad.isFullQuad())
        {
            return null;
        ...
        else
        {
            ...
            try
            {
                TextureAtlasSprite textureatlassprite = getConnectedTextureRepeat(cp, 阻止位置, side);
    ...
    private static BakedQuad[] getConnectedTextureOverlayCtm(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, 阻止位置 阻止位置, int vertAxis, int side, BakedQuad quad, int metadata, RenderEnv renderEnv)
    {
        if (!quad.isFullQuad())
        {
            return null;
        ...
        else
        {
            ...
            try
            {
                TextureAtlasSprite textureatlassprite = getConnectedTextureCtm(cp, blockAccess, blockState, 阻止位置, vertAxis, side, quad.getSprite(), metadata, renderEnv);
    ...
    private static int getConnectedTextureCtmIndex(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, 阻止位置 阻止位置, int vertAxis, int side, TextureAtlasSprite icon, int metadata, RenderEnv renderEnv)
    {
        boolean[] aboolean = renderEnv.getBorderFlags();
        ...
        {
            case 0:
                aboolean[0] = isNeighbour(cp, blockAccess, blockState, 阻止位置.west(), side, icon, metadata);
                aboolean[1] = isNeighbour(cp, blockAccess, blockState, 阻止位置.east(), side, icon, metadata);
                aboolean[2] = isNeighbour(cp, blockAccess, blockState, 阻止位置.north(), side, icon, metadata);
                aboolean[3] = isNeighbour(cp, blockAccess, blockState, 阻止位置.south(), side, icon, metadata);

                if (cp.innerSeams)
                {
                    阻止位置 blockpos6 = 阻止位置.down();
            ...
            case 1:
                aboolean[0] = isNeighbour(cp, blockAccess, blockState, 阻止位置.west(), side, icon, metadata);
                aboolean[1] = isNeighbour(cp, blockAccess, blockState, 阻止位置.east(), side, icon, metadata);
                aboolean[2] = isNeighbour(cp, blockAccess, blockState, 阻止位置.south(), side, icon, metadata);
                aboolean[3] = isNeighbour(cp, blockAccess, blockState, 阻止位置.north(), side, icon, metadata);

                if (cp.innerSeams)
                {
                    阻止位置 blockpos5 = 阻止位置.up();
            ...
            case 2:
                aboolean[0] = isNeighbour(cp, blockAccess, blockState, 阻止位置.east(), side, icon, metadata);
                aboolean[1] = isNeighbour(cp, blockAccess, blockState, 阻止位置.west(), side, icon, metadata);
                aboolean[2] = isNeighbour(cp, blockAccess, blockState, 阻止位置.down(), side, icon, metadata);
                aboolean[3] = isNeighbour(cp, blockAccess, blockState, 阻止位置.up(), side, icon, metadata);

                if (cp.innerSeams)
                {
                    阻止位置 blockpos4 = 阻止位置.north();
            ...
            case 4:
                aboolean[0] = isNeighbour(cp, blockAccess, blockState, 阻止位置.north(), side, icon, metadata);
                aboolean[1] = isNeighbour(cp, blockAccess, blockState, 阻止位置.south(), side, icon, metadata);
                aboolean[2] = isNeighbour(cp, blockAccess, blockState, 阻止位置.down(), side, icon, metadata);
                aboolean[3] = isNeighbour(cp, blockAccess, blockState, 阻止位置.up(), side, icon, metadata);

                if (cp.innerSeams)
                {
                    阻止位置 blockpos2 = 阻止位置.west();
            ...
            case 5:
                aboolean[0] = isNeighbour(cp, blockAccess, blockState, 阻止位置.south(), side, icon, metadata);
                aboolean[1] = isNeighbour(cp, blockAccess, blockState, 阻止位置.north(), side, icon, metadata);
                aboolean[2] = isNeighbour(cp, blockAccess, blockState, 阻止位置.down(), side, icon, metadata);
                aboolean[3] = isNeighbour(cp, blockAccess, blockState, 阻止位置.up(), side, icon, metadata);

                if (cp.innerSeams)
                {
                    阻止位置 blockpos = 阻止位置.east();
        ...
        else
        {
            ...
            {
                case 0:
                    aboolean[0] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.east().north(), side, icon, metadata);
                    aboolean[1] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.west().north(), side, icon, metadata);
                    aboolean[2] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.east().south(), side, icon, metadata);
                    aboolean[3] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.west().south(), side, icon, metadata);

                    if (cp.innerSeams)
                    {
                        阻止位置 blockpos11 = 阻止位置.down();
                ...
                case 1:
                    aboolean[0] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.east().south(), side, icon, metadata);
                    aboolean[1] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.west().south(), side, icon, metadata);
                    aboolean[2] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.east().north(), side, icon, metadata);
                    aboolean[3] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.west().north(), side, icon, metadata);

                    if (cp.innerSeams)
                    {
                        阻止位置 blockpos10 = 阻止位置.up();
                ...
                case 2:
                    aboolean[0] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.west().down(), side, icon, metadata);
                    aboolean[1] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.east().down(), side, icon, metadata);
                    aboolean[2] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.west().up(), side, icon, metadata);
                    aboolean[3] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.east().up(), side, icon, metadata);

                    if (cp.innerSeams)
                    {
                        阻止位置 blockpos9 = 阻止位置.north();
                ...
                case 3:
                    aboolean[0] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.east().down(), side, icon, metadata);
                    aboolean[1] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.west().down(), side, icon, metadata);
                    aboolean[2] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.east().up(), side, icon, metadata);
                    aboolean[3] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.west().up(), side, icon, metadata);

                    if (cp.innerSeams)
                    {
                        阻止位置 blockpos8 = 阻止位置.south();
                ...
                case 4:
                    aboolean[0] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.down().south(), side, icon, metadata);
                    aboolean[1] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.down().north(), side, icon, metadata);
                    aboolean[2] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.up().south(), side, icon, metadata);
                    aboolean[3] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.up().north(), side, icon, metadata);

                    if (cp.innerSeams)
                    {
                        阻止位置 blockpos7 = 阻止位置.west();
                ...
                case 5:
                    aboolean[0] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.down().north(), side, icon, metadata);
                    aboolean[1] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.down().south(), side, icon, metadata);
                    aboolean[2] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.up().north(), side, icon, metadata);
                    aboolean[3] = !isNeighbour(cp, blockAccess, blockState, 阻止位置.up().south(), side, icon, metadata);

                    if (cp.innerSeams)
                    {
                        阻止位置 blockpos1 = 阻止位置.east();
    ...
    private static boolean isNeighbourOverlay(ConnectedProperties cp, IBlockAccess iblockaccess, IBlockState blockState, 阻止位置 阻止位置, int side, TextureAtlasSprite icon, int metadata)
    {
        IBlockState iblockstate = iblockaccess.getBlockState(阻止位置);
        ...
        else
        {
            ...
            {
                TextureAtlasSprite textureatlassprite = getNeighbourIcon(iblockaccess, blockState, 阻止位置, iblockstate, side);
            ...
            IBlockState iblockstate1 = iblockaccess.getBlockState(阻止位置.offset(getFacing(side)));
            return iblockstate1.getBlock().isOpaqueCube() ? false : (side == 1 && iblockstate1.getBlock() == Blocks.snow_layer ? false : !isNeighbour(cp, iblockaccess, blockState, 阻止位置, iblockstate, side, icon, metadata));
        }
    }
    ...
    private static boolean isNeighbourMatching(ConnectedProperties cp, IBlockAccess iblockaccess, IBlockState blockState, 阻止位置 阻止位置, int side, TextureAtlasSprite icon, int metadata)
    {
        IBlockState iblockstate = iblockaccess.getBlockState(阻止位置);
    ...
    private static boolean isNeighbour(ConnectedProperties cp, IBlockAccess iblockaccess, IBlockState blockState, 阻止位置 阻止位置, int side, TextureAtlasSprite icon, int metadata)
    {
        IBlockState iblockstate = iblockaccess.getBlockState(阻止位置);
        return isNeighbour(cp, iblockaccess, blockState, 阻止位置, iblockstate, side, icon, metadata);
    }

    private static boolean isNeighbour(ConnectedProperties cp, IBlockAccess iblockaccess, IBlockState blockState, 阻止位置 阻止位置, IBlockState neighbourState, int side, TextureAtlasSprite icon, int metadata)
    {
        ...
        else if (cp.connect == 2)
        {
            ...
            {
                TextureAtlasSprite textureatlassprite = getNeighbourIcon(iblockaccess, blockState, 阻止位置, neighbourState, side);
    ...
    private static TextureAtlasSprite getNeighbourIcon(IBlockAccess iblockaccess, IBlockState blockState, 阻止位置 阻止位置, IBlockState neighbourState, int side)
    {
        neighbourState = neighbourState.getBlock().getActualState(neighbourState, iblockaccess, 阻止位置);
        IBakedModel ibakedmodel = 我的手艺.得到我的手艺().getBlockRendererDispatcher().getBlockModelShapes().getModelForState(neighbourState);
        ...
        else
        {
            ...
            {
                neighbourState = (IBlockState)Reflector.call(neighbourState.getBlock(), Reflector.ForgeBlock_getExtendedState, new Object[] {neighbourState, iblockaccess, 阻止位置});
            }
            ...
            else
            {
                ...
                {
                    list = BetterGrass.getFaceQuads(iblockaccess, neighbourState, 阻止位置, enumfacing, list);
                }
    ...
    private static TextureAtlasSprite getConnectedTextureHorizontal(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, 阻止位置 阻止位置, int vertAxis, int side, TextureAtlasSprite icon, int metadata)
    {
        boolean flag;
        boolean flag1;
        flag = false;
        ...
        switch (vertAxis)
        {
            case 0:
                ...
                {
                    case 0:
                        flag = isNeighbour(cp, blockAccess, blockState, 阻止位置.west(), side, icon, metadata);
                        flag1 = isNeighbour(cp, blockAccess, blockState, 阻止位置.east(), side, icon, metadata);
                        break label0;
                    ...
                    case 2:
                        flag = isNeighbour(cp, blockAccess, blockState, 阻止位置.east(), side, icon, metadata);
                        flag1 = isNeighbour(cp, blockAccess, blockState, 阻止位置.west(), side, icon, metadata);
                        break label0;
                    ...
                    case 4:
                        flag = isNeighbour(cp, blockAccess, blockState, 阻止位置.north(), side, icon, metadata);
                        flag1 = isNeighbour(cp, blockAccess, blockState, 阻止位置.south(), side, icon, metadata);
                        break label0;

                    case 5:
                        flag = isNeighbour(cp, blockAccess, blockState, 阻止位置.south(), side, icon, metadata);
                        flag1 = isNeighbour(cp, blockAccess, blockState, 阻止位置.north(), side, icon, metadata);
            ...
            case 1:
                switch (side)
                {
                    ...
                    case 4:
                        flag = isNeighbour(cp, blockAccess, blockState, 阻止位置.down(), side, icon, metadata);
                        flag1 = isNeighbour(cp, blockAccess, blockState, 阻止位置.up(), side, icon, metadata);
                        break label0;

                    case 5:
                        flag = isNeighbour(cp, blockAccess, blockState, 阻止位置.up(), side, icon, metadata);
                        flag1 = isNeighbour(cp, blockAccess, blockState, 阻止位置.down(), side, icon, metadata);
    ...
    private static TextureAtlasSprite getConnectedTextureVertical(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, 阻止位置 阻止位置, int vertAxis, int side, TextureAtlasSprite icon, int metadata)
    {
        boolean flag = false;
        boolean flag1 = false;
    ...
    private static TextureAtlasSprite getConnectedTextureHorizontalVertical(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, 阻止位置 阻止位置, int vertAxis, int side, TextureAtlasSprite icon, int metadata)
    {
        TextureAtlasSprite[] atextureatlassprite = cp.tileIcons;
        TextureAtlasSprite textureatlassprite = getConnectedTextureHorizontal(cp, blockAccess, blockState, 阻止位置, vertAxis, side, icon, metadata);
        ...
        {
            TextureAtlasSprite textureatlassprite1 = getConnectedTextureVertical(cp, blockAccess, blockState, 阻止位置, vertAxis, side, icon, metadata);
    ...
    private static TextureAtlasSprite getConnectedTextureVerticalHorizontal(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, 阻止位置 阻止位置, int vertAxis, int side, TextureAtlasSprite icon, int metadata)
    {
        TextureAtlasSprite[] atextureatlassprite = cp.tileIcons;
        TextureAtlasSprite textureatlassprite = getConnectedTextureVertical(cp, blockAccess, blockState, 阻止位置, vertAxis, side, icon, metadata);
        ...
        {
            TextureAtlasSprite textureatlassprite1 = getConnectedTextureHorizontal(cp, blockAccess, blockState, 阻止位置, vertAxis, side, icon, metadata);
            return textureatlassprite1 == atextureatlassprite[0] ? atextureatlassprite[4] : (textureatlassprite1 == atextureatlassprite[1] ? atextureatlassprite[5] : (textureatlassprite1 == atextureatlassprite[2] ? atextureatlassprite[6] : textureatlassprite1));
        }
    }

    private static TextureAtlasSprite getConnectedTextureTop(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, 阻止位置 阻止位置, int vertAxis, int side, TextureAtlasSprite icon, int metadata)
    {
        boolean flag = false;
