...
public class BehaviorDefaultDispenseItem implements IBehaviorDispenseItem
{
    ...
    public static void doDispense(World worldIn, ItemStack stack, int speed, EnumFacing facing, IPosition position)
    {
        ...
        实体Item entityitem = new 实体Item(worldIn, d0, d1, d2, stack);
        double d3 = worldIn.rand.nextDouble() * 0.1D + 0.2D;
        entityitem.通便X = (double)facing.getFrontOffsetX() * d3;
        entityitem.motionY = 0.20000000298023224D;
        entityitem.通便Z = (double)facing.getFrontOffsetZ() * d3;
        entityitem.通便X += worldIn.rand.nextGaussian() * 0.007499999832361937D * (double)speed;
        entityitem.motionY += worldIn.rand.nextGaussian() * 0.007499999832361937D * (double)speed;
        entityitem.通便Z += worldIn.rand.nextGaussian() * 0.007499999832361937D * (double)speed;
