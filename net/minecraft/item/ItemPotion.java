        ...
        if (stack.getMetadata() != 0)
        {
            ...
            if (list != null && !list.isEmpty())
            {
                for (PotionEffect potioneffect : list)
                {
                    ...
                    {
                        tooltip.add(枚举聊天格式.RED + s1);
                    }
                    else
                    {
                        tooltip.add(枚举聊天格式.GRAY + s1);
                    }
                }
            }
            else
            {
                ...
                tooltip.add(枚举聊天格式.GRAY + s);
            }

            if (!multimap.isEmpty())
            {
                tooltip.add("");
                tooltip.add(枚举聊天格式.DARK_PURPLE + StatCollector.translateToLocal("potion.effects.whenDrank"));
                ...
                for (Entry<String, AttributeModifier> entry1 : multimap.entries())
                {
                    ...
                    if (d0 > 0.0D)
                    {
                        tooltip.add(枚举聊天格式.BLUE + StatCollector.translateToLocalFormatted("attribute.modifier.plus." + attributemodifier2.getOperation(), new Object[] {ItemStack.DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + (String)entry1.getKey())}));
                    }
                    else if (d0 < 0.0D)
                    {
                        d1 = d1 * -1.0D;
                        tooltip.add(枚举聊天格式.RED + StatCollector.translateToLocalFormatted("attribute.modifier.take." + attributemodifier2.getOperation(), new Object[] {ItemStack.DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + (String)entry1.getKey())}));
                    }
                }
            }
        }
