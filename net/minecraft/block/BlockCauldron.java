...
public class BlockCauldron extends Block
{
    ...
    public void onEntityCollidedWithBlock(World worldIn, 阻止位置 pos, IBlockState state, 实体 实体In)
    {
        ...
        if (!worldIn.isRemote && 实体In.isBurning() && i > 0 && 实体In.getEntityBoundingBox().minY <= (double)f)
        {
            实体In.extinguish();
                ...
                else if (item == Items.glass_bottle)
                {
                    if (i > 0)
                    {
                        if (!playerIn.capabilities.isCreativeMode)
                        {
                            ...
                            {
                                worldIn.spawnEntityInWorld(new 实体Item(worldIn, (double)pos.getX() + 0.5D, (double)pos.getY() + 1.5D, (double)pos.getZ() + 0.5D, itemstack2));
                            }
                            else if (playerIn instanceof 实体PlayerMP)
                            {
                                ((实体PlayerMP)playerIn).sendContainerToPlayer(playerIn.inventoryContainer);
                            }
                ...
                else
                {
                    ...
                    if (i > 0 && item instanceof ItemBanner && TileEntityBanner.getPatterns(itemstack) > 0)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                worldIn.spawnEntityInWorld(new 实体Item(worldIn, (double)pos.getX() + 0.5D, (double)pos.getY() + 1.5D, (double)pos.getZ() + 0.5D, itemstack1));
                            }
                            else if (playerIn instanceof 实体PlayerMP)
                            {
                                ((实体PlayerMP)playerIn).sendContainerToPlayer(playerIn.inventoryContainer);
    ...
    public void setWaterLevel(World worldIn, 阻止位置 pos, IBlockState state, int level)
    {
