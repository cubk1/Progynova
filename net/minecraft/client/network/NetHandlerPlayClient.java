...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    private final 鬼Screen guiScreenServer;
    private 我的手艺 gameController;
    ...
    public NetHandlerPlayClient(我的手艺 mcIn, 鬼Screen p_i46300_2_, NetworkManager p_i46300_3_, GameProfile p_i46300_4_)
    {
    ...
    public void handleJoinGame(S01PacketJoinGame packetIn)
    {
        ...
        this.gameController.玩家控制者 = new PlayerControllerMP(this.gameController, this);
        ...
        this.gameController.游戏一窝.difficulty = packetIn.getDifficulty();
        this.gameController.loadWorld(this.clientWorldController);
        this.gameController.宇轩游玩者.dimension = packetIn.getDimension();
        this.gameController.displayGuiScreen(new 鬼DownloadTerrain(this));
        this.gameController.宇轩游玩者.setEntityId(packetIn.getEntityId());
        this.currentServerMaxPlayers = packetIn.getMaxPlayers();
        this.gameController.宇轩游玩者.setReducedDebug(packetIn.isReducedDebugInfo());
        this.gameController.玩家控制者.setGameType(packetIn.getGameType());
        this.gameController.游戏一窝.sendSettingsToServer();
    ...
    public void handleHeldItemChange(S09PacketHeldItemChange packetIn)
    {
        ...
        {
            this.gameController.宇轩游玩者.inventory.currentItem = packetIn.getHeldItemHotbarIndex();
        }
    }
    ...
    public void onDisconnect(IChatComponent reason)
    {
        ...
        {
            if (this.guiScreenServer instanceof 鬼ScreenRealmsProxy)
            {
                this.gameController.displayGuiScreen((new DisconnectedRealmsScreen(((鬼ScreenRealmsProxy)this.guiScreenServer).func_154321_a(), "disconnect.lost", reason)).getProxy());
            }
            else
            {
                this.gameController.displayGuiScreen(new 鬼Disconnected(this.guiScreenServer, "disconnect.lost", reason));
            }
        }
        else
        {
            this.gameController.displayGuiScreen(new 鬼Disconnected(new 鬼Multiplayer(new 鬼MainMenu()), "disconnect.lost", reason));
        }
    }
    ...
    public void handleSpawnMob(S0FPacketSpawnMob packetIn)
    {
        ...
        entitylivingbase.通便X = (double)((float)packetIn.getVelocityX() / 8000.0F);
        entitylivingbase.motionY = (double)((float)packetIn.getVelocityY() / 8000.0F);
        entitylivingbase.通便Z = (double)((float)packetIn.getVelocityZ() / 8000.0F);
    ...
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        this.gameController.宇轩の世界.setTotalWorldTime(packetIn.getTotalWorldTime());
        this.gameController.宇轩の世界.setWorldTime(packetIn.getWorldTime());
    }

    public void handleSpawnPosition(S05PacketSpawnPosition packetIn)
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        this.gameController.宇轩游玩者.setSpawnPoint(packetIn.getSpawnPos(), true);
        this.gameController.宇轩の世界.getWorldInfo().setSpawn(packetIn.getSpawnPos());
    }
    ...
    public void handleEntityAttach(S1BPacketEntityAttach packetIn)
    {
        ...
        if (packetIn.getLeash() == 0)
        {
            ...
            if (flag)
            {
                GameSettings gamesettings = this.gameController.游戏一窝;
    ...
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        this.gameController.宇轩游玩者.setPlayerSPHealth(packetIn.getHealth());
        this.gameController.宇轩游玩者.获取饥饿值().setFoodLevel(packetIn.getFoodLevel());
        this.gameController.宇轩游玩者.获取饥饿值().setFoodSaturationLevel(packetIn.getSaturationLevel());
    }

    public void handleSetExperience(S1FPacketSetExperience packetIn)
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        this.gameController.宇轩游玩者.setXPStats(packetIn.func_149397_c(), packetIn.getTotalExperience(), packetIn.getLevel());
    }
    ...
    public void handleRespawn(S07PacketRespawn packetIn)
    {
        ...
        if (packetIn.getDimensionID() != this.gameController.宇轩游玩者.dimension)
        {
            this.doneLoadingTerrain = false;
            Scoreboard scoreboard = this.clientWorldController.getScoreboard();
            this.clientWorldController = new WorldClient(this, new WorldSettings(0L, packetIn.getGameType(), false, this.gameController.宇轩の世界.getWorldInfo().isHardcoreModeEnabled(), packetIn.getWorldType()), packetIn.getDimensionID(), packetIn.getDifficulty(), this.gameController.mcProfiler);
            this.clientWorldController.setWorldScoreboard(scoreboard);
            this.gameController.loadWorld(this.clientWorldController);
            this.gameController.宇轩游玩者.dimension = packetIn.getDimensionID();
            this.gameController.displayGuiScreen(new 鬼DownloadTerrain(this));
        }
    ...
    public void handleExplosion(S27PacketExplosion packetIn)
    {
        ...
        this.gameController.宇轩游玩者.通便Z += (double)packetIn.func_149147_e();
    }
    ...
    public void handleSetSlot(S2FPacketSetSlot packetIn)
    {
        ...
        else
        {
            ...
            if (this.gameController.currentScreen instanceof 鬼ContainerCreative)
            {
                鬼ContainerCreative guicontainercreative = (鬼ContainerCreative)this.gameController.currentScreen;
    ...
    public void handleSignEditorOpen(S36PacketSignEditorOpen packetIn)
    {
        ...
        this.gameController.宇轩游玩者.openEditSign((TileEntitySign)tileentity);
    }
    ...
    public void handleUpdateSign(S33PacketUpdateSign packetIn)
    {
        ...
        if (this.gameController.宇轩の世界.isBlockLoaded(packetIn.getPos()))
        {
            TileEntity tileentity = this.gameController.宇轩の世界.getTileEntity(packetIn.getPos());
        ...
        if (!flag && this.gameController.宇轩游玩者 != null)
        {
            this.gameController.宇轩游玩者.增添聊天讯息(new 交流组分文本("Unable to locate sign at " + packetIn.getPos().getX() + ", " + packetIn.getPos().getY() + ", " + packetIn.getPos().getZ()));
        }
    }
    ...
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        this.gameController.宇轩游玩者.closeScreenAndDropStack();
    }

    public void handleBlockAction(S24PacketBlockAction packetIn)
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        this.gameController.宇轩の世界.addBlockEvent(packetIn.getBlockPosition(), packetIn.getBlockType(), packetIn.getData1(), packetIn.getData2());
    }

    public void handleBlockBreakAnim(S25PacketBlockBreakAnim packetIn)
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        this.gameController.宇轩の世界.sendBlockBreakProgress(packetIn.getBreakerId(), packetIn.getPosition(), packetIn.getProgress());
    }
    ...
    public void handleChangeGameState(S2BPacketChangeGameState packetIn)
    {
        ...
        {
            this.gameController.玩家控制者.setGameType(WorldSettings.GameType.getByID(j));
        }
        else if (i == 4)
        {
            this.gameController.displayGuiScreen(new 鬼WinGame());
        }
        else if (i == 5)
        {
            GameSettings gamesettings = this.gameController.游戏一窝;
    ...
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        MapData mapdata = ItemMap.loadMapData(packetIn.getMapId(), this.gameController.宇轩の世界);
    ...
    public void handleEffect(S28PacketEffect packetIn)
    {
        ...
        {
            this.gameController.宇轩の世界.playBroadcastSound(packetIn.getSoundType(), packetIn.getSoundPos(), packetIn.getSoundData());
        }
        else
        {
            this.gameController.宇轩の世界.playAuxSFX(packetIn.getSoundType(), packetIn.getSoundPos(), packetIn.getSoundData());
        }
    }
    ...
    public void handleStatistics(S37PacketStatistics packetIn)
    {
        ...
        for (Entry<StatBase, Integer> entry : packetIn.func_148974_c().entrySet())
        {
            ...
            {
                if (this.field_147308_k && this.gameController.宇轩游玩者.getStatFileWriter().readStat(statbase) == 0)
                {
                    ...
                    {
                        this.gameController.游戏一窝.showInventoryAchievementHint = false;
                        this.gameController.游戏一窝.saveOptions();
                    }
                }
            ...
            this.gameController.宇轩游玩者.getStatFileWriter().unlockAchievement(this.gameController.宇轩游玩者, statbase, i);
        }

        if (!this.field_147308_k && !flag && this.gameController.游戏一窝.showInventoryAchievementHint)
        {
    ...
    public void handleCombatEvent(S42PacketCombatEvent packetIn)
    {
        ...
        if (packetIn.eventType == S42PacketCombatEvent.Event.END_COMBAT)
        {
            ...
            MetadataCombat metadatacombat = new MetadataCombat(this.gameController.宇轩游玩者, entitylivingbase);
    ...
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        this.gameController.宇轩の世界.getWorldInfo().setDifficulty(packetIn.getDifficulty());
        this.gameController.宇轩の世界.getWorldInfo().setDifficultyLocked(packetIn.isDifficultyLocked());
    }
    ...
    public void handleTabComplete(S3APacketTabComplete packetIn)
    {
        ...
        if (this.gameController.currentScreen instanceof 鬼Chat)
        {
            鬼Chat guichat = (鬼Chat)this.gameController.currentScreen;
    ...
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        this.gameController.宇轩の世界.playSound(packetIn.getX(), packetIn.getY(), packetIn.getZ(), packetIn.getSoundName(), packetIn.getVolume(), packetIn.getPitch(), false);
    }
        ...
        else
        {
            ...
            else
            {
                this.gameController.addScheduledTask(new Runnable()
                {
                    public void run()
                    {
                        NetHandlerPlayClient.this.gameController.displayGuiScreen(new 鬼YesNo(new GuiYesNoCallback()
                        {
                            public void confirmClicked(boolean result, int id)
                            {
                                NetHandlerPlayClient.this.gameController = 我的手艺.得到我的手艺();
                                ...
                                NetHandlerPlayClient.this.gameController.displayGuiScreen((鬼Screen)null);
                            }
    ...
    public void handleCustomPayload(S3FPacketCustomPayload packetIn)
    {
        ...
        if ("MC|TrList".equals(packetIn.getChannelName()))
        {
            ...
            try
            {
                ...
                鬼Screen guiscreen = this.gameController.currentScreen;

                if (guiscreen != null && guiscreen instanceof 鬼Merchant && i == this.gameController.宇轩游玩者.openContainer.windowId)
                {
                    IMerchant imerchant = ((鬼Merchant)guiscreen).getMerchant();
        ...
        {
            this.gameController.宇轩游玩者.setClientBrand(packetIn.getBufferData().readStringFromBuffer(32767));
        }
        else if ("MC|BOpen".equals(packetIn.getChannelName()))
        {
            ItemStack itemstack = this.gameController.宇轩游玩者.getCurrentEquippedItem();

            if (itemstack != null && itemstack.getItem() == Items.written_book)
            {
                this.gameController.displayGuiScreen(new 鬼ScreenBook(this.gameController.宇轩游玩者, itemstack, false));
            }
        }
    }
