...
public abstract class ResourcePackListEntry implements GuiListExtended.IGuiListEntry
{
    ...
    protected final 鬼ScreenResourcePacks resourcePacksGUI;

    public ResourcePackListEntry(鬼ScreenResourcePacks resourcePacksGUIIn)
    {
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected)
    {
        ...
        if (i != 1)
        {
            ...
            鬼.drawRect(x - 1, y - 1, x + listWidth - 9, y + slotHeight + 1, -8978432);
        }

        this.func_148313_c();
        光照状态经理.色彩(1.0F, 1.0F, 1.0F, 1.0F);
        鬼.绘制模态矩形以自定义大小纹理(x, y, 0.0F, 0.0F, 32, 32, 32.0F, 32.0F);
        String s = this.func_148312_b();
        String s1 = this.func_148311_a();

        if ((this.mc.游戏一窝.touchscreen || isSelected) && this.func_148310_d())
        {
            this.mc.得到手感经理().绑定手感(RESOURCE_PACKS_TEXTURE);
            ...
            else
            {
                if (this.func_148308_f())
                {
                    if (j < 16)
                    {
                        鬼.绘制模态矩形以自定义大小纹理(x, y, 32.0F, 32.0F, 32, 32, 256.0F, 256.0F);
                    }
                    else
                    {
                        鬼.绘制模态矩形以自定义大小纹理(x, y, 32.0F, 0.0F, 32, 32, 256.0F, 256.0F);
                    }
                }
        ...
        if (this.func_148310_d() && p_148278_5_ <= 32)
        {
            if (this.func_148309_e())
            {
                ...
                if (j != 1)
                {
                    ...
                    this.mc.displayGuiScreen(new 鬼YesNo(new GuiYesNoCallback()
                    {
