...
public class 鬼LockIconButton extends 鬼Button
{
    private boolean field_175231_o = false;

    public 鬼LockIconButton(int p_i45538_1_, int p_i45538_2_, int p_i45538_3_)
    {
    ...
    public void drawButton(我的手艺 mc, int mouseX, int mouseY)
    {
        if (this.visible)
        {
            ...
            鬼LockIconButton.Icon guilockiconbutton$icon;

            if (this.field_175231_o)
            {
                if (!this.enabled)
                {
                    guilockiconbutton$icon = 鬼LockIconButton.Icon.LOCKED_DISABLED;
                }
                else if (flag)
                {
                    guilockiconbutton$icon = 鬼LockIconButton.Icon.LOCKED_HOVER;
                }
                else
                {
                    guilockiconbutton$icon = 鬼LockIconButton.Icon.LOCKED;
                }
            }
            else if (!this.enabled)
            {
                guilockiconbutton$icon = 鬼LockIconButton.Icon.UNLOCKED_DISABLED;
            }
            else if (flag)
            {
                guilockiconbutton$icon = 鬼LockIconButton.Icon.UNLOCKED_HOVER;
            }
            else
            {
                guilockiconbutton$icon = 鬼LockIconButton.Icon.UNLOCKED;
            }
