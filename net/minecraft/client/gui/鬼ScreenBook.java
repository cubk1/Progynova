...
public class 鬼ScreenBook extends 鬼Screen
{
    ...
    private 鬼ScreenBook.NextPageButton buttonNextPage;
    private 鬼ScreenBook.NextPageButton buttonPreviousPage;
    private 鬼Button buttonDone;
    private 鬼Button buttonSign;
    ...
    public void initGui()
    {
        ...
        {
            this.buttonList.add(this.buttonSign = new 鬼Button(3, this.width / 2 - 100, 4 + this.bookImageHeight, 98, 20, I18n.format("book.signButton", new Object[0])));
            this.buttonList.add(this.buttonDone = new 鬼Button(0, this.width / 2 + 2, 4 + this.bookImageHeight, 98, 20, I18n.format("gui.done", new Object[0])));
            this.buttonList.add(this.buttonFinalize = new 鬼Button(5, this.width / 2 - 100, 4 + this.bookImageHeight, 98, 20, I18n.format("book.finalizeButton", new Object[0])));
            this.buttonList.add(this.buttonCancel = new 鬼Button(4, this.width / 2 + 2, 4 + this.bookImageHeight, 98, 20, I18n.format("gui.cancel", new Object[0])));
        }
        else
        {
            this.buttonList.add(this.buttonDone = new 鬼Button(0, this.width / 2 - 100, 4 + this.bookImageHeight, 200, 20, I18n.format("gui.done", new Object[0])));
        }

        int i = (this.width - this.bookImageWidth) / 2;
        int j = 2;
        this.buttonList.add(this.buttonNextPage = new 鬼ScreenBook.NextPageButton(1, i + 120, j + 154, true));
        this.buttonList.add(this.buttonPreviousPage = new 鬼ScreenBook.NextPageButton(2, i + 38, j + 154, false));
        this.updateButtons();
    }
            ...
            if (this.bookPages != null)
            {
                ...
                if (publish)
                {
                    ...
                    for (int i = 0; i < this.bookPages.tagCount(); ++i)
                    {
                        ...
                        IChatComponent ichatcomponent = new 交流组分文本(s1);
    ...
    {
        if (鬼Screen.isKeyComboCtrlV(keyCode))
        {
            this.pageInsertIntoCurrent(鬼Screen.getClipboardString());
        }
        else
        {
            switch (keyCode)
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        this.mc.得到手感经理().绑定手感(bookGuiTextures);
        int i = (this.width - this.bookImageWidth) / 2;
        int j = 2;
    ...
    static class NextPageButton extends 鬼Button
    {
        ...
        public void drawButton(我的手艺 mc, int mouseX, int mouseY)
        {
            if (this.visible)
            {
                ...
                光照状态经理.色彩(1.0F, 1.0F, 1.0F, 1.0F);
                mc.得到手感经理().绑定手感(鬼ScreenBook.bookGuiTextures);
                int i = 0;
                int j = 192;
