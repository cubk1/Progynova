...
{
    private final 鬼Controls field_148191_k;
    private final 我的手艺 mc;
    private final GuiListExtended.IGuiListEntry[] listEntries;
    private int maxListLabelWidth = 0;

    public GuiKeyBindingList(鬼Controls controls, 我的手艺 mcIn)
    {
        super(mcIn, controls.width, controls.height, 63, controls.height - 32, 20);
        this.field_148191_k = controls;
        this.mc = mcIn;
        键入绑定[] akeybinding = (键入绑定[])ArrayUtils.clone(mcIn.游戏一窝.键入绑定s);
        this.listEntries = new GuiListExtended.IGuiListEntry[akeybinding.length + 键入绑定.getKeybinds().size()];
        Arrays.sort((Object[])akeybinding);
        int i = 0;
        String s = null;

        for (键入绑定 keybinding : akeybinding)
        {
    ...
    {
        private final 键入绑定 keybinding;
        private final String keyDesc;
        private final 鬼Button btnChangeKeyBinding;
        private final 鬼Button btnReset;

        private KeyEntry(键入绑定 p_i45029_2_)
        {
            this.keybinding = p_i45029_2_;
            this.keyDesc = I18n.format(p_i45029_2_.getKeyDescription(), new Object[0]);
            this.btnChangeKeyBinding = new 鬼Button(0, 0, 0, 75, 20, I18n.format(p_i45029_2_.getKeyDescription(), new Object[0]));
            this.btnReset = new 鬼Button(0, 0, 0, 50, 20, I18n.format("controls.reset", new Object[0]));
        }
        ...
        public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected)
        {
            ...
            {
                for (键入绑定 keybinding : GuiKeyBindingList.this.mc.游戏一窝.键入绑定s)
                {
        ...
        public boolean mousePressed(int slotIndex, int p_148278_2_, int p_148278_3_, int p_148278_4_, int p_148278_5_, int p_148278_6_)
        {
            ...
            {
                GuiKeyBindingList.this.mc.游戏一窝.setOptionKeyBinding(this.keybinding, this.keybinding.getKeyCodeDefault());
