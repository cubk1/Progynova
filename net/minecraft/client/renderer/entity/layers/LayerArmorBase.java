        ...
        if (itemstack != null && itemstack.getItem() instanceof ItemArmor)
        {
            ...
            if (Reflector.ForgeHooksClient_getArmorTexture.exists())
            {
                if (ReflectorForge.armorHasOverlay(itemarmor, itemstack))
                {
                    ...
                    光照状态经理.色彩(this.colorR * f3, this.colorG * f4, this.colorB * f5, this.alpha);
                ...
                光照状态经理.色彩(this.colorR, this.colorG, this.colorB, this.alpha);
                t.render(entitylivingbaseIn, p_177182_2_, p_177182_3_, p_177182_5_, p_177182_6_, p_177182_7_, scale);
            ...
            switch (itemarmor.getArmorMaterial())
            {
                case LEATHER:
                    ...
                    光照状态经理.色彩(this.colorR * f, this.colorG * f1, this.colorB * f2, this.alpha);
                    t.render(entitylivingbaseIn, p_177182_2_, p_177182_3_, p_177182_5_, p_177182_6_, p_177182_7_, scale);
    ...
    private void renderGlint(EntityLivingBase entitylivingbaseIn, T modelbaseIn, float p_177183_3_, float p_177183_4_, float partialTicks, float p_177183_6_, float p_177183_7_, float p_177183_8_, float scale)
    {
        if (!Config.isShaders() || !Shaders.isShadowPass)
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                ...
                光照状态经理.正常工作(768, 1);
                float f2 = 0.76F;
                光照状态经理.色彩(0.5F * f2, 0.25F * f2, 0.8F * f2, 1.0F);
                光照状态经理.matrixMode(5890);
                光照状态经理.loadIdentity();
                float f3 = 0.33333334F;
                光照状态经理.障眼物(f3, f3, f3);
                光照状态经理.辐射(30.0F - (float)i * 60.0F, 0.0F, 0.0F, 1.0F);
                光照状态经理.理解(0.0F, f * (0.001F + (float)i * 0.003F) * 20.0F, 0.0F);
