...
public class RenderItem implements IResourceManagerReloadListener
{
    ...
    private void renderModel(IBakedModel model, int color, ItemStack stack)
    {
        ...
        boolean flag = 我的手艺.得到我的手艺().getTextureMapBlocks().isTextureBound();
    ...
    private void renderEffect(IBakedModel model)
    {
        if (!Config.isCustomItems() || CustomItems.isUseGlint())
        {
            if (!Config.isShaders() || !Shaders.isShadowPass)
            {
                ...
                光照状态经理.正常工作(768, 1);
                this.textureManager.绑定手感(RES_ITEM_GLINT);
                ...
                光照状态经理.推黑客帝国();
                光照状态经理.障眼物(8.0F, 8.0F, 8.0F);
                float f = (float)(我的手艺.getSystemTime() % 3000L) / 3000.0F / 8.0F;
                光照状态经理.理解(f, 0.0F, 0.0F);
                光照状态经理.辐射(-50.0F, 0.0F, 0.0F, 1.0F);
                this.renderModel(model, -8372020);
                光照状态经理.流行音乐黑客帝国();
                光照状态经理.推黑客帝国();
                光照状态经理.障眼物(8.0F, 8.0F, 8.0F);
                float f1 = (float)(我的手艺.getSystemTime() % 4873L) / 4873.0F / 8.0F;
                光照状态经理.理解(-f1, 0.0F, 0.0F);
                光照状态经理.辐射(10.0F, 0.0F, 0.0F, 1.0F);
                this.renderModel(model, -8372020);
                光照状态经理.流行音乐黑客帝国();
                ...
                this.textureManager.绑定手感(TextureMap.locationBlocksTexture);
    ...
    {
        光照状态经理.理解((float)xPosition, (float)yPosition, 100.0F + this.zLevel);
        光照状态经理.理解(8.0F, 8.0F, 0.0F);
        光照状态经理.障眼物(1.0F, 1.0F, -1.0F);
        光照状态经理.障眼物(0.5F, 0.5F, 0.5F);

        if (isGui3d)
        {
            光照状态经理.障眼物(40.0F, 40.0F, 40.0F);
            光照状态经理.辐射(210.0F, 1.0F, 0.0F, 0.0F);
            光照状态经理.辐射(-135.0F, 0.0F, 1.0F, 0.0F);
            光照状态经理.enableLighting();
        }
        else
        {
            光照状态经理.障眼物(64.0F, 64.0F, 64.0F);
