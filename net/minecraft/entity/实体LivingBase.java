...
public abstract class 实体LivingBase extends 实体
{
    ...
    protected 实体Player attackingPlayer;
    ...
    private 实体LivingBase entityLivingToAttack;
    private int revengeTimer;
    private 实体LivingBase lastAttacker;
    ...
    public 实体LivingBase(World worldIn)
    {
        super(worldIn);
        ...
        this.旋转侧滑 = (float)(Math.random() * Math.PI * 2.0D);
    ...
    protected void updateFallState(double y, boolean onGroundIn, Block blockIn, 阻止位置 pos)
    {
        ...
        if (!this.worldObj.isRemote && this.fallDistance > 3.0F && onGroundIn)
        {
            ...
            if (block.getMaterial() != Material.air)
            {
                ...
                int i = (int)(150.0D * d0);
                ((WorldServer)this.worldObj).spawnParticle(EnumParticleTypes.BLOCK_DUST, this.X坐标, this.Y坐标, this.Z坐标, i, 0.0D, 0.0D, 0.0D, 0.15000000596046448D, new int[] {Block.getStateId(iblockstate)});
            }
        }
    ...
    public void onEntityUpdate()
    {
        ...
        boolean flag = this instanceof 实体Player;
        ...
        boolean flag1 = flag && ((实体Player)this).capabilities.disableDamage;
        ...
        if (this.isEntityAlive())
        {
            if (this.isInsideOfMaterial(Material.water))
            {
                if (!this.canBreatheUnderwater() && !this.isPotionActive(Potion.waterBreathing.id) && !flag1)
                {
                    ...
                    if (this.getAir() == -20)
                    {
                        ...
                        for (int i = 0; i < 8; ++i)
                        {
                            ...
                            this.worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.X坐标 + (double)f, this.Y坐标 + (double)f1, this.Z坐标 + (double)f2, this.通便X, this.motionY, this.通便Z, new int[0]);
                        }
                ...
                if (!this.worldObj.isRemote && this.isRiding() && this.riding实体 instanceof 实体LivingBase)
                {
        ...
        if (this.hurtResistantTime > 0 && !(this instanceof 实体PlayerMP))
        {
        ...
        if (this.entityLivingToAttack != null)
        {
            ...
            {
                this.setRevengeTarget((实体LivingBase)null);
            }
    ...
    protected void onDeathUpdate()
    {
        ...
        if (this.deathTime == 20)
        {
            ...
            for (int k = 0; k < 20; ++k)
            {
                ...
                this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, this.X坐标 + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, this.Y坐标 + (double)(this.rand.nextFloat() * this.height), this.Z坐标 + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, d2, d0, d1, new int[0]);
            }
        }
    }
    ...
    public 实体LivingBase getAITarget()
    {
    ...
    public 实体LivingBase getLastAttacker()
    {
    ...
    public void setLastAttacker(实体 实体In)
    {
        if (实体In instanceof 实体LivingBase)
        {
            this.lastAttacker = (实体LivingBase) 实体In;
        }
        else
        {
            this.lastAttacker = null;
    ...
    protected void updatePotionEffects()
    {
        ...
        if (i > 0)
        {
            ...
            if (flag && i > 0)
            {
                ...
                this.worldObj.spawnParticle(flag1 ? EnumParticleTypes.SPELL_MOB_AMBIENT : EnumParticleTypes.SPELL_MOB, this.X坐标 + (this.rand.nextDouble() - 0.5D) * (double)this.width, this.Y坐标 + this.rand.nextDouble() * (double)this.height, this.Z坐标 + (this.rand.nextDouble() - 0.5D) * (double)this.width, d0, d1, d2, new int[0]);
            }
        }
    }
        ...
        else
        {
            ...
            else
            {
                ...
                if (实体 != null)
                {
                    ...
                    {
                        this.setRevengeTarget((实体LivingBase) 实体);
                    }

                    if (实体 instanceof 实体Player)
                    {
                        this.recentlyHit = 100;
                        this.attackingPlayer = (实体Player) 实体;
                    }
                    else if (实体 instanceof 实体Wolf)
                    {
                        实体Wolf entitywolf = (实体Wolf) 实体;
                ...
                if (flag)
                {
                    ...
                    if (实体 != null)
                    {
                        double d1 = 实体.X坐标 - this.X坐标;
                        double d0;

                        for (d0 = 实体.Z坐标 - this.Z坐标; d1 * d1 + d0 * d0 < 1.0E-4D; d0 = (Math.random() - Math.random()) * 0.01D)
                        {
                            d1 = (Math.random() - Math.random()) * 0.01D;
                        }

                        this.attackedAtYaw = (float)(MathHelper.atan2(d0, d1) * 180.0D / Math.PI - (double)this.旋转侧滑);
                        this.knockBack(实体, amount, d1, d0);
                    }
                    else
                    {
    ...
    public void onDeath(DamageSource cause)
    {
        ...
        实体LivingBase entitylivingbase = this.getAttackingEntity();
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (实体 instanceof 实体Player)
            {
                i = EnchantmentHelper.getLootingModifier((实体LivingBase) 实体);
            }
    ...
    public void knockBack(实体 实体In, float p_70653_2_, double p_70653_3_, double p_70653_5_)
    {
        if (this.rand.nextDouble() >= this.getEntityAttribute(SharedMonsterAttributes.knockbackResistance).getAttributeValue())
        {
            ...
            this.motionY /= 2.0D;
            this.通便Z /= 2.0D;
            this.通便X -= p_70653_3_ / (double)f * (double)f1;
            this.motionY += (double)f1;
            this.通便Z -= p_70653_5_ / (double)f * (double)f1;
    ...
    public boolean isOnLadder()
    {
        ...
        Block block = this.worldObj.getBlockState(new 阻止位置(i, j, k)).getBlock();
        return (block == Blocks.ladder || block == Blocks.vine) && (!(this instanceof 实体Player) || !((实体Player)this).isSpectator());
    }
    ...
    public void fall(float distance, float damageMultiplier)
    {
        ...
        if (i > 0)
        {
            ...
            int k = MathHelper.floor_double(this.Y坐标 - 0.20000000298023224D);
            int l = MathHelper.floor_double(this.Z坐标);
            Block block = this.worldObj.getBlockState(new 阻止位置(j, k, l)).getBlock();
    ...
    public 实体LivingBase getAttackingEntity()
    {
        return (实体LivingBase)(this._combatTracker.func_94550_c() != null ? this._combatTracker.func_94550_c() : (this.attackingPlayer != null ? this.attackingPlayer : (this.entityLivingToAttack != null ? this.entityLivingToAttack : null)));
    }
    ...
    public void dismountEntity(实体 实体In)
    {
        double d0 = 实体In.X坐标;
        double d1 = 实体In.getEntityBoundingBox().minY + (double) 实体In.height;
        double d2 = 实体In.Z坐标;
        int i = 1;
        ...
        for (int j = -i; j <= i; ++j)
        {
            for (int k = -i; k < i; ++k)
            {
                if (j != 0 || k != 0)
                {
                    int l = (int)(this.X坐标 + (double)j);
                    int i1 = (int)(this.Z坐标 + (double)k);
                    ...
                    {
                        if (World.doesBlockHaveSolidTopSurface(this.worldObj, new 阻止位置(l, (int)this.Y坐标, i1)))
                        {
                            this.setPositionAndUpdate(this.X坐标 + (double)j, this.Y坐标 + 1.0D, this.Z坐标 + (double)k);
                            return;
                        }

                        if (World.doesBlockHaveSolidTopSurface(this.worldObj, new 阻止位置(l, (int)this.Y坐标 - 1, i1)) || this.worldObj.getBlockState(new 阻止位置(l, (int)this.Y坐标 - 1, i1)).getBlock().getMaterial() == Material.water)
                        {
                            d0 = this.X坐标 + (double)j;
                            d1 = this.Y坐标 + 1.0D;
                            d2 = this.Z坐标 + (double)k;
                        }
                    }
                }
            }
    ...
    protected void jump()
    {
        ...
        if (this.isSprinting())
        {
            float f = this.旋转侧滑 * 0.017453292F;
            this.通便X -= (double)(MathHelper.sin(f) * 0.2F);
            this.通便Z += (double)(MathHelper.cos(f) * 0.2F);
        }
    ...
    public void moveEntityWithHeading(float strafe, float forward)
    {
        ...
        {
            if (!this.isInWater() || this instanceof 实体Player && ((实体Player)this).capabilities.isFlying)
            {
                if (!this.isInLava() || this instanceof 实体Player && ((实体Player)this).capabilities.isFlying)
                {
                    float f4 = 0.91F;

                    if (this.onGround)
                    {
                        f4 = this.worldObj.getBlockState(new 阻止位置(MathHelper.floor_double(this.X坐标), MathHelper.floor_double(this.getEntityBoundingBox().minY) - 1, MathHelper.floor_double(this.Z坐标))).getBlock().slipperiness * 0.91F;
                    }
                    ...
                    {
                        float f6 = 0.15F;
                        this.通便X = MathHelper.clamp_double(this.通便X, (double)(-f6), (double)f6);
                        this.通便Z = MathHelper.clamp_double(this.通便Z, (double)(-f6), (double)f6);
                        ...
                        boolean flag = this.正在下蹲() && this instanceof 实体Player;
                    ...
                    if (this.worldObj.isRemote && (!this.worldObj.isBlockLoaded(new 阻止位置((int)this.X坐标, 0, (int)this.Z坐标)) || !this.worldObj.getChunkFromBlockCoords(new 阻止位置((int)this.X坐标, 0, (int)this.Z坐标)).isLoaded()))
                    {
                        if (this.Y坐标 > 0.0D)
                        {
                            this.motionY = -0.1D;
                ...
                else
                {
                    ...
                    if (this.isCollidedHorizontally && this.isOffsetPositionInLiquid(this.通便X, this.motionY + 0.6000000238418579D - this.Y坐标 + d1, this.通便Z))
                    {
            ...
            else
            {
                ...
                if (this.isCollidedHorizontally && this.isOffsetPositionInLiquid(this.通便X, this.motionY + 0.6000000238418579D - this.Y坐标 + d0, this.通便Z))
                {
                    this.motionY = 0.30000001192092896D;
                }
            }
        ...
        double d2 = this.X坐标 - this.prevPosX;
        double d3 = this.Z坐标 - this.prevPosZ;
    ...
    protected float updateDistance(float p_110146_1_, float p_110146_2_)
    {
        ...
        float f1 = MathHelper.wrapAngleTo180_float(this.旋转侧滑 - this.renderYawOffset);
        ...
        this.renderYawOffset = this.旋转侧滑 - f1;
    ...
    public void onLivingUpdate()
    {
        ...
        {
            double d0 = this.X坐标 + (this.newPosX - this.X坐标) / (double)this.newPosRotationIncrements;
            double d1 = this.Y坐标 + (this.newPosY - this.Y坐标) / (double)this.newPosRotationIncrements;
            double d2 = this.Z坐标 + (this.newPosZ - this.Z坐标) / (double)this.newPosRotationIncrements;
            double d3 = MathHelper.wrapAngleTo180_double(this.newRotationYaw - (double)this.旋转侧滑);
            this.旋转侧滑 = (float)((double)this.旋转侧滑 + d3 / (double)this.newPosRotationIncrements);
    ...
    {
        List<实体> list = this.worldObj.getEntitiesInAABBexcluding(this, this.getEntityBoundingBox().expand(0.20000000298023224D, 0.0D, 0.20000000298023224D), Predicates.<实体> and (EntitySelectors.NOT_SPECTATING, new Predicate<实体>()
        {
    ...
    {
        实体In.applyEntityCollision(this);
    }

    public void mountEntity(实体 实体In)
    {
        if (this.riding实体 != null && 实体In == null)
        {
            if (!this.worldObj.isRemote)
            {
                this.dismountEntity(this.riding实体);
            }
    ...
    public boolean canEntityBeSeen(实体 实体In)
    {
        return this.worldObj.rayTraceBlocks(new Vec3(this.X坐标, this.Y坐标 + (double)this.getEyeHeight(), this.Z坐标), new Vec3(实体In.X坐标, 实体In.Y坐标 + (double) 实体In.getEyeHeight(), 实体In.Z坐标)) == null;
    }
