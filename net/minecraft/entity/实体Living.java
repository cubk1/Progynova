...
public abstract class 实体Living extends 实体LivingBase
{
    ...
    public 实体LivingBase getAttackTarget()
    {
        return this.attackTarget;
    }

    public void setAttackTarget(实体LivingBase entitylivingbaseIn)
    {
    ...
    public void spawnExplosionParticle()
    {
        if (this.worldObj.isRemote)
        {
            for (int i = 0; i < 20; ++i)
            {
                ...
                double d3 = 10.0D;
                this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, this.X坐标 + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width - d0 * d3, this.Y坐标 + (double)(this.rand.nextFloat() * this.height) - d1 * d3, this.Z坐标 + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width - d2 * d3, d0, d1, d2, new int[0]);
            }
        }
        else
        {
    ...
    public void writeEntityToNBT(NBTTagCompound tagCompound)
    {
        ...
        if (this.leashedTo实体 != null)
        {
            NBTTagCompound nbttagcompound1 = new NBTTagCompound();

            if (this.leashedTo实体 instanceof 实体LivingBase)
            {
                nbttagcompound1.setLong("UUIDMost", this.leashedTo实体.getUniqueID().getMostSignificantBits());
                nbttagcompound1.setLong("UUIDLeast", this.leashedTo实体.getUniqueID().getLeastSignificantBits());
            }
            else if (this.leashedTo实体 instanceof 实体Hanging)
            {
                阻止位置 blockpos = ((实体Hanging)this.leashedTo实体).getHangingPosition();
    ...
    public void onLivingUpdate()
    {
        ...
        {
            for (实体Item entityitem : this.worldObj.getEntitiesWithinAABB(实体Item.class, this.getEntityBoundingBox().expand(1.0D, 0.0D, 1.0D)))
            {
    ...
    protected void updateEquipmentIfNeeded(实体Item itemEntity)
    {
        ...
        if (i > -1)
        {
            ...
            if (flag && this.func_175448_a(itemstack))
            {
                ...
                {
                    实体Player entityplayer = this.worldObj.getPlayerEntityByName(itemEntity.getThrower());
    ...
    protected void despawnEntity()
    {
        ...
        {
            实体 实体 = this.worldObj.getClosestPlayerToEntity(this, -1.0D);

            if (实体 != null)
            {
                double d0 = 实体.X坐标 - this.X坐标;
                double d1 = 实体.Y坐标 - this.Y坐标;
                double d2 = 实体.Z坐标 - this.Z坐标;
    ...
    public void faceEntity(实体 实体In, float p_70625_2_, float p_70625_3_)
    {
        ...
        {
            实体LivingBase entitylivingbase = (实体LivingBase) 实体In;
            d2 = entitylivingbase.Y坐标 + (double)entitylivingbase.getEyeHeight() - (this.Y坐标 + (double)this.getEyeHeight());
        }
        else
        {
            d2 = (实体In.getEntityBoundingBox().minY + 实体In.getEntityBoundingBox().maxY) / 2.0D - (this.Y坐标 + (double)this.getEyeHeight());
        }
        ...
        this.旋转侧滑 = this.updateRotation(this.旋转侧滑, f, p_70625_2_);
    }
    ...
    public final boolean interactFirst(实体Player playerIn)
    {
        ...
        else
        {
            ...
            {
                if (!(this instanceof 实体Tameable) || !((实体Tameable)this).isTamed())
                {
                ...
                if (((实体Tameable)this).isOwner(playerIn))
                {
                    this.setLeashedToEntity(playerIn, true);
                    --itemstack.stackSize;
                    return true;
    ...
    protected void updateLeashedState()
    {
        ...
        if (this.isLeashed)
        {
            ...
            if (this.leashedTo实体 == null || this.leashedTo实体.isDead)
            {
                this.clearLeashed(true, true);
            }
        }
    ...
    public void clearLeashed(boolean sendPacket, boolean dropLead)
    {
        if (this.isLeashed)
        {
            ...
            {
                ((WorldServer)this.worldObj).getEntityTracker().sendToAllTrackingEntity(this, new S1BPacketEntityAttach(1, this, (实体)null));
            }
        }
    }
    ...
    public 实体 getLeashedToEntity()
    {
        return this.leashedTo实体;
    }

    public void setLeashedToEntity(实体 实体In, boolean sendAttachNotification)
    {
        this.isLeashed = true;
        this.leashedTo实体 = 实体In;

        if (!this.worldObj.isRemote && sendAttachNotification && this.worldObj instanceof WorldServer)
        {
            ((WorldServer)this.worldObj).getEntityTracker().sendToAllTrackingEntity(this, new S1BPacketEntityAttach(1, this, this.leashedTo实体));
        }
    }
    ...
    private void recreateLeash()
    {
        if (this.isLeashed && this.leashNBTTag != null)
        {
            if (this.leashNBTTag.hasKey("UUIDMost", 4) && this.leashNBTTag.hasKey("UUIDLeast", 4))
            {
                ...
                for (实体LivingBase entitylivingbase : this.worldObj.getEntitiesWithinAABB(实体LivingBase.class, this.getEntityBoundingBox().expand(10.0D, 10.0D, 10.0D)))
                {
                    if (entitylivingbase.getUniqueID().equals(uuid))
                    {
                        this.leashedTo实体 = entitylivingbase;
                        break;
                    }
                }
            }
            ...
            {
                阻止位置 blockpos = new 阻止位置(this.leashNBTTag.getInteger("X"), this.leashNBTTag.getInteger("Y"), this.leashNBTTag.getInteger("Z"));
                实体LeashKnot entityleashknot = 实体LeashKnot.getKnotForPosition(this.worldObj, blockpos);

                if (entityleashknot == null)
                {
                    entityleashknot = 实体LeashKnot.createKnot(this.worldObj, blockpos);
                }

                this.leashedTo实体 = entityleashknot;
            }
            else
            {
    ...
    private boolean canSkipUpdate()
    {
        ...
        else
        {
            ...
            {
                实体 实体 = (实体)world.playerEntities.get(0);
                double d0 = Math.max(Math.abs(this.X坐标 - 实体.X坐标) - 16.0D, 0.0D);
                double d1 = Math.max(Math.abs(this.Z坐标 - 实体.Z坐标) - 16.0D, 0.0D);
