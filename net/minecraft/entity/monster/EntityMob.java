...
public abstract class EntityMob extends EntityCreature implements IMob
{
    ...
    public boolean attackEntityAsMob(Entity entityIn)
    {
        ...
        if (flag)
        {
            if (i > 0)
            {
                entityIn.addVelocity((double)(-MathHelper.sin(this.旋转侧滑 * (float)Math.PI / 180.0F) * (float)i * 0.5F), 0.1D, (double)(MathHelper.cos(this.旋转侧滑 * (float)Math.PI / 180.0F) * (float)i * 0.5F));
                this.通便X *= 0.6D;
                this.通便Z *= 0.6D;
            }
