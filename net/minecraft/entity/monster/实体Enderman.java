...
public class 实体Enderman extends 实体Mob
{
    ...
    public 实体Enderman(World worldIn)
    {
        super(worldIn);
        ...
        this.tasks.addTask(10, new 实体Enderman.AIPlaceBlock(this));
        this.tasks.addTask(11, new 实体Enderman.AITakeBlock(this));
        this.targetTasks.addTask(1, new EntityAIHurtByTarget(this, false, new Class[0]));
        this.targetTasks.addTask(2, new 实体Enderman.AIFindPlayer(this));
        this.targetTasks.addTask(3, new EntityAINearestAttackableTarget(this, 实体Endermite.class, 10, true, false, new Predicate<实体Endermite>()
        {
            public boolean apply(实体Endermite p_apply_1_)
            {
    ...
    private boolean shouldAttackPlayer(实体Player player)
    {
        ...
        else
        {
            ...
            Vec3 vec31 = new Vec3(this.X坐标 - player.X坐标, this.getEntityBoundingBox().minY + (double)(this.height / 2.0F) - (player.Y坐标 + (double)player.getEyeHeight()), this.Z坐标 - player.Z坐标);
    ...
    public void onLivingUpdate()
    {
        if (this.worldObj.isRemote)
        {
            for (int i = 0; i < 2; ++i)
            {
                this.worldObj.spawnParticle(EnumParticleTypes.PORTAL, this.X坐标 + (this.rand.nextDouble() - 0.5D) * (double)this.width, this.Y坐标 + this.rand.nextDouble() * (double)this.height - 0.25D, this.Z坐标 + (this.rand.nextDouble() - 0.5D) * (double)this.width, (this.rand.nextDouble() - 0.5D) * 2.0D, -this.rand.nextDouble(), (this.rand.nextDouble() - 0.5D) * 2.0D, new int[0]);
            }
        }
    ...
    protected void updateAITasks()
    {
        ...
        if (this.worldObj.isDaytime())
        {
            ...
            if (f > 0.5F && this.worldObj.canSeeSky(new 阻止位置(this)) && this.rand.nextFloat() * 30.0F < (f - 0.4F) * 2.0F)
            {
                this.setAttackTarget((实体LivingBase)null);
                this.setScreaming(false);
    ...
    {
        double d0 = this.X坐标 + (this.rand.nextDouble() - 0.5D) * 64.0D;
        double d1 = this.Y坐标 + (double)(this.rand.nextInt(64) - 32);
        double d2 = this.Z坐标 + (this.rand.nextDouble() - 0.5D) * 64.0D;
        return this.teleportTo(d0, d1, d2);
    }

    protected boolean teleportToEntity(实体 p_70816_1_)
    {
        Vec3 vec3 = new Vec3(this.X坐标 - p_70816_1_.X坐标, this.getEntityBoundingBox().minY + (double)(this.height / 2.0F) - p_70816_1_.Y坐标 + (double)p_70816_1_.getEyeHeight(), this.Z坐标 - p_70816_1_.Z坐标);
        vec3 = vec3.normalize();
        double d0 = 16.0D;
        double d1 = this.X坐标 + (this.rand.nextDouble() - 0.5D) * 8.0D - vec3.xCoord * d0;
        double d2 = this.Y坐标 + (double)(this.rand.nextInt(16) - 8) - vec3.yCoord * d0;
        double d3 = this.Z坐标 + (this.rand.nextDouble() - 0.5D) * 8.0D - vec3.zCoord * d0;
    ...
    protected boolean teleportTo(double x, double y, double z)
    {
        ...
        this.Z坐标 = z;
        boolean flag = false;
        阻止位置 blockpos = new 阻止位置(this.X坐标, this.Y坐标, this.Z坐标);
        ...
        if (this.worldObj.isBlockLoaded(blockpos))
        {
            ...
            if (flag1)
            {
                super.setPositionAndUpdate(this.X坐标, this.Y坐标, this.Z坐标);
        ...
        else
        {
            ...
            for (int j = 0; j < i; ++j)
            {
                ...
                double d3 = d0 + (this.X坐标 - d0) * d6 + (this.rand.nextDouble() - 0.5D) * (double)this.width * 2.0D;
                double d4 = d1 + (this.Y坐标 - d1) * d6 + this.rand.nextDouble() * (double)this.height;
                double d5 = d2 + (this.Z坐标 - d2) * d6 + (this.rand.nextDouble() - 0.5D) * (double)this.width * 2.0D;
    ...
    public boolean attackEntityFrom(DamageSource source, float amount)
    {
        ...
        {
            if (source.getEntity() == null || !(source.getEntity() instanceof 实体Endermite))
            {
                if (!this.worldObj.isRemote)
                {
                ...
                if (source instanceof EntityDamageSource && source.getEntity() instanceof 实体Player)
                {
                    if (source.getEntity() instanceof 实体PlayerMP && ((实体PlayerMP)source.getEntity()).theItemInWorldManager.isCreative())
                    {
                        this.setScreaming(false);
                    }
                    else
    ...
    static class AIFindPlayer extends EntityAINearestAttackableTarget
    {
        ...
        private 实体Enderman enderman;

        public AIFindPlayer(实体Enderman p_i45842_1_)
        {
            super(p_i45842_1_, 实体Player.class, true);
        ...
        public boolean shouldExecute()
        {
            ...
            List<实体Player> list = this.taskOwner.worldObj.<实体Player>getEntitiesWithinAABB(实体Player.class, this.taskOwner.getEntityBoundingBox().expand(d0, 4.0D, d0), this.targetEntitySelector);
            ...
            {
                this.player = (实体Player)list.get(0);
                return true;
            }
        }
        ...
        public void resetTask()
        {
            ...
            iattributeinstance.removeModifier(实体Enderman.attackingSpeedBoostModifier);
            super.resetTask();
        }
        ...
        public void updateTask()
        {
            if (this.player != null)
            {
                if (--this.field_179450_h <= 0)
                {
                    ...
                    IAttributeInstance iattributeinstance = this.enderman.getEntityAttribute(SharedMonsterAttributes.movementSpeed);
                    iattributeinstance.applyModifier(实体Enderman.attackingSpeedBoostModifier);
                }
            }
            else
            {
                ...
                {
                    if (this.targetEntity instanceof 实体Player && this.enderman.shouldAttackPlayer((实体Player)this.targetEntity))
                    {
    ...
    static class AIPlaceBlock extends EntityAIBase
    {
        ...
        public AIPlaceBlock(实体Enderman p_i45843_1_)
        {
        ...
        public void updateTask()
        {
            ...
            int i = MathHelper.floor_double(this.enderman.X坐标 - 1.0D + random.nextDouble() * 2.0D);
            int j = MathHelper.floor_double(this.enderman.Y坐标 + random.nextDouble() * 2.0D);
            int k = MathHelper.floor_double(this.enderman.Z坐标 - 1.0D + random.nextDouble() * 2.0D);
        ...
        private boolean func_179474_a(World worldIn, 阻止位置 p_179474_2_, Block p_179474_3_, Block p_179474_4_, Block p_179474_5_)
        {
    ...
    static class AITakeBlock extends EntityAIBase
    {
        ...
        public AITakeBlock(实体Enderman p_i45841_1_)
        {
        ...
        public void updateTask()
        {
            Random random = this.enderman.getRNG();
            World world = this.enderman.worldObj;
            int i = MathHelper.floor_double(this.enderman.X坐标 - 2.0D + random.nextDouble() * 4.0D);
            int j = MathHelper.floor_double(this.enderman.Y坐标 + random.nextDouble() * 3.0D);
            int k = MathHelper.floor_double(this.enderman.Z坐标 - 2.0D + random.nextDouble() * 4.0D);
            阻止位置 blockpos = new 阻止位置(i, j, k);
            IBlockState iblockstate = world.getBlockState(blockpos);
            Block block = iblockstate.getBlock();

            if (实体Enderman.carriableBlocks.contains(block))
            {
