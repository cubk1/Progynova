...
public class 实体Witch extends 实体Mob implements IRangedAttackMob
{
    ...
    public 实体Witch(World worldIn)
    {
        ...
        this.tasks.addTask(3, new EntityAIWatchClosest(this, 实体Player.class, 8.0F));
    ...
    public void handleStatusUpdate(byte id)
    {
        if (id == 15)
        {
            ...
            {
                this.worldObj.spawnParticle(EnumParticleTypes.SPELL_WITCH, this.X坐标 + this.rand.nextGaussian() * 0.12999999523162842D, this.getEntityBoundingBox().maxY + 0.5D + this.rand.nextGaussian() * 0.12999999523162842D, this.Z坐标 + this.rand.nextGaussian() * 0.12999999523162842D, 0.0D, 0.0D, 0.0D, new int[0]);
            }
        }
        else
        {
    ...
    public void attackEntityWithRangedAttack(实体LivingBase target, float p_82196_2_)
    {
        if (!this.getAggressive())
        {
            实体Potion entitypotion = new 实体Potion(this.worldObj, this, 32732);
            double d0 = target.Y坐标 + (double)target.getEyeHeight() - 1.100000023841858D;
            entitypotion.rotationPitch -= -20.0F;
            double d1 = target.X坐标 + target.通便X - this.X坐标;
            double d2 = d0 - this.Y坐标;
            double d3 = target.Z坐标 + target.通便Z - this.Z坐标;
