...
public class 实体PlayerMP extends 实体Player implements ICrafting
{
    ...
    private 实体 spectating实体 = null;
    private int currentWindowId;
    public boolean isChangingQuantityOnly;
    public int ping;
    public boolean playerConqueredTheEnd;

    public 实体PlayerMP(MinecraftServer server, WorldServer worldIn, GameProfile profile, ItemInWorldManager interactionManager)
    {
        ...
        while (!worldIn.getCollidingBoundingBoxes(this, this.getEntityBoundingBox()).isEmpty() && this.Y坐标 < 255.0D)
        {
            this.setPosition(this.X坐标, this.Y坐标 + 1.0D, this.Z坐标);
        }
    }
    ...
    public void onUpdate()
    {
        ...
        if (!this.loadedChunks.isEmpty())
        {
            ...
            while (iterator1.hasNext() && ((List)list).size() < 10)
            {
                ...
                {
                    if (this.worldObj.isBlockLoaded(new 阻止位置(chunkcoordintpair.chunkXPos << 4, 0, chunkcoordintpair.chunkZPos << 4)))
                    {
        ...
        实体 实体 = this.getSpectatingEntity();
        ...
        if (实体 != this)
        {
            ...
            {
                this.setPositionAndRotation(实体.X坐标, 实体.Y坐标, 实体.Z坐标, 实体.旋转侧滑, 实体.rotationPitch);
    ...
    public void onUpdateEntity()
    {
        try
        {
            ...
            if (this.getHealth() != this.lastHealth || this.lastFoodLevel != this.foodStats.获取饥饿等级() || this.foodStats.getSaturationLevel() == 0.0F != this.wasHungry)
            {
                this.playerNetServerHandler.sendPacket(new S06PacketUpdateHealth(this.getHealth(), this.foodStats.获取饥饿等级(), this.foodStats.getSaturationLevel()));
                this.lastHealth = this.getHealth();
                this.lastFoodLevel = this.foodStats.获取饥饿等级();
            ...
            if (this.getHealth() + this.getAbsorptionAmount() != this.combinedHealth)
            {
                ...
                {
                    this.getWorldScoreboard().getValueFromObjective(this.getName(), scoreobjective).func_96651_a(Arrays.<实体Player>asList(new 实体Player[] {this}));
                }
            }
    ...
    {
        BiomeGenBase biomegenbase = this.worldObj.getBiomeGenForCoords(new 阻止位置(MathHelper.floor_double(this.X坐标), 0, MathHelper.floor_double(this.Z坐标)));
        ...
        else
        {
            ...
            else
            {
                if (source instanceof EntityDamageSource)
                {
                    ...
                    if (实体 instanceof 实体Player && !this.canAttackPlayer((实体Player) 实体))
                    {
                        return false;
                    }
                    ...
                    if (实体 instanceof 实体Arrow)
                    {
                        ...
                        if (entityarrow.shooting实体 instanceof 实体Player && !this.canAttackPlayer((实体Player)entityarrow.shooting实体))
                        {
                            return false;
                        }
                    }
    ...
    public void travelToDimension(int dimensionId)
    {
        ...
        else
        {
            if (this.dimension == 0 && dimensionId == 1)
            {
                ...
                阻止位置 blockpos = this.mcServer.worldServerForDimension(dimensionId).getSpawnCoordinate();
    ...
    {
        实体Player.EnumStatus entityplayer$enumstatus = super.trySleep(bedLocation);
        ...
        if (entityplayer$enumstatus == 实体Player.EnumStatus.OK)
        {
            ...
            this.playerNetServerHandler.setPlayerLocation(this.X坐标, this.Y坐标, this.Z坐标, this.旋转侧滑, this.rotationPitch);
            this.playerNetServerHandler.sendPacket(packet);
        }
    ...
    public void mountEntity(实体 实体In)
    {
        ...
        if (实体In != 实体)
        {
            this.playerNetServerHandler.sendPacket(new S1BPacketEntityAttach(0, this, this.riding实体));
            this.playerNetServerHandler.setPlayerLocation(this.X坐标, this.Y坐标, this.Z坐标, this.旋转侧滑, this.rotationPitch);
        }
    }
    ...
    public void displayGUIChest(IInventory chestInventory)
    {
        ...
        if (chestInventory instanceof ILockableContainer)
        {
            ...
            if (ilockablecontainer.isLocked() && !this.canOpen(ilockablecontainer.getLockCode()) && !this.isSpectator())
            {
                ...
                this.playerNetServerHandler.sendPacket(new S29PacketSoundEffect("random.door_close", this.X坐标, this.Y坐标, this.Z坐标, 1.0F, 1.0F));
                return;
            }
        }
    ...
    public void clonePlayer(实体Player oldPlayer, boolean respawnFromEnd)
    {
        ...
        this.lastHealth = -1.0F;
        this.lastFoodLevel = -1;
        this.destroyedItemsNetCache.addAll(((实体PlayerMP)oldPlayer).destroyedItemsNetCache);
    }
    ...
    {
        this.playerNetServerHandler.setPlayerLocation(x, y, z, this.旋转侧滑, this.rotationPitch);
    }

    public void onCriticalHit(实体 实体Hit)
    {
        this.getServerForPlayer().getEntityTracker().func_151248_b(this, new S0BPacketAnimation(实体Hit, 4));
    }

    public void onEnchantmentCritical(实体 实体Hit)
    {
        this.getServerForPlayer().getEntityTracker().func_151248_b(this, new S0BPacketAnimation(实体Hit, 5));
    }
    ...
    public void removeEntity(实体 p_152339_1_)
    {
        if (p_152339_1_ instanceof 实体Player)
        {
    ...
    public 实体 getSpectatingEntity()
    {
        return (实体)(this.spectating实体 == null ? this : this.spectating实体);
    }

    public void setSpectatingEntity(实体 实体ToSpectate)
    {
        实体 实体 = this.getSpectatingEntity();
        this.spectating实体 = (实体)(实体ToSpectate == null ? this : 实体ToSpectate);

        if (实体 != this.spectating实体)
        {
            this.playerNetServerHandler.sendPacket(new S43PacketCamera(this.spectating实体));
            this.setPositionAndUpdate(this.spectating实体.X坐标, this.spectating实体.Y坐标, this.spectating实体.Z坐标);
        }
    }
    ...
    public void attackTargetEntityWithCurrentItem(实体 target实体)
    {
        ...
        {
            this.setSpectatingEntity(target实体);
        }
        else
        {
            super.attackTargetEntityWithCurrentItem(target实体);
        }
    }
