        ...
        if (targetEntity.canAttackWithItem())
        {
            if (!targetEntity.hitByEntity(this))
            {
                ...
                if (f > 0.0F || f1 > 0.0F)
                {
                    ...
                    double d0 = targetEntity.通便X;
                    double d1 = targetEntity.motionY;
                    double d2 = targetEntity.通便Z;
                    ...
                    if (flag2)
                    {
                        if (i > 0)
                        {
                            targetEntity.addVelocity(-MathHelper.sin(this.旋转侧滑 * (float)Math.PI / 180.0F) * (float)i * 0.5F, 0.1D, MathHelper.cos(this.旋转侧滑 * (float)Math.PI / 180.0F) * (float)i * 0.5F);
                            this.通便X *= 0.6D;
                            this.通便Z *= 0.6D;
