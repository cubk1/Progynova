...
import net.minecraft.entity.monster.实体Mob;
...
public abstract class 实体Player extends 实体LivingBase
{
    ...
    public 阻止位置 playerLocation;
    private int sleepTimer;
    public float renderOffsetX;
    public float renderOffsetY;
    public float renderOffsetZ;
    private 阻止位置 spawnChunk;
    private boolean spawnForced;
    private 阻止位置 startMinecartRidingCoordinate;
    ...
    public 实体FishHook fishEntity;

    public 实体Player(World worldIn, GameProfile gameProfileIn)
    {
        super(worldIn);
    ...
    public void onUpdate()
    {
        ...
        double d5 = this.X坐标 - this.chasingPosX;
        double d0 = this.Y坐标 - this.chasingPosY;
        double d1 = this.Z坐标 - this.chasingPosZ;
        double d2 = 10.0D;

        if (d5 > d2)
        {
            this.prevChasingPosX = this.chasingPosX = this.X坐标;
        }

        if (d1 > d2)
        {
            this.prevChasingPosZ = this.chasingPosZ = this.Z坐标;
        }

        if (d0 > d2)
        {
            this.prevChasingPosY = this.chasingPosY = this.Y坐标;
        }
        ...
        double d3 = MathHelper.clamp_double(this.X坐标, -2.9999999E7D, 2.9999999E7D);
        double d4 = MathHelper.clamp_double(this.Z坐标, -2.9999999E7D, 2.9999999E7D);

        if (d3 != this.X坐标 || d4 != this.Z坐标)
        {
            this.setPosition(d3, this.Y坐标, d4);
        }
    }
    ...
    protected void updateItemUse(ItemStack itemStackIn, int p_71010_2_)
    {
        ...
        if (itemStackIn.getItemUseAction() == EnumAction.EAT)
        {
            for (int i = 0; i < p_71010_2_; ++i)
            {
                ...
                vec3 = vec3.rotateYaw(-this.旋转侧滑 * (float)Math.PI / 180.0F);
                ...
                vec31 = vec31.rotateYaw(-this.旋转侧滑 * (float)Math.PI / 180.0F);
                vec31 = vec31.addVector(this.X坐标, this.Y坐标 + (double)this.getEyeHeight(), this.Z坐标);
    ...
    {
        if (!this.worldObj.isRemote && this.正在下蹲())
        {
            this.mountEntity((实体)null);
        ...
        else
        {
            ...
            this.cameraYaw = 0.0F;
            this.addMountedMovementStat(this.X坐标 - d0, this.Y坐标 - d1, this.Z坐标 - d2);

            if (this.riding实体 instanceof 实体Pig)
            {
                this.rotationPitch = f1;
                this.旋转侧滑 = f;
                this.renderYawOffset = ((实体Pig)this.riding实体).renderYawOffset;
            }
        }
    }
    ...
    protected void updateEntityActionState()
    {
        ...
        this.rotationYawHead = this.旋转侧滑;
    }
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.getHealth() > 0.0F && !this.isSpectator())
        {
            ...
            {
                axisalignedbb = this.getEntityBoundingBox().union(this.riding实体.getEntityBoundingBox()).expand(1.0D, 0.0D, 1.0D);
            }
            else
            {
            ...
            List<实体> list = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, axisalignedbb);

            for (实体 实体 : list) {
                if (!实体.isDead) {
                    this.collideWithPlayer(实体);
                }
            }
        }
    }

    private void collideWithPlayer(实体 p_71044_1_)
    {
    ...
    public void onDeath(DamageSource cause)
    {
        ...
        this.通便X = -MathHelper.cos((this.attackedAtYaw + this.旋转侧滑) * (float) Math.PI / 180.0F) * 0.1F;
        this.通便Z = -MathHelper.sin((this.attackedAtYaw + this.旋转侧滑) * (float)Math.PI / 180.0F) * 0.1F;
    ...
    public void addToPlayerScore(实体 实体In, int amount)
    {
        ...
        if (实体In instanceof 实体Player)
        {
            ...
            collection.addAll(this.func_175137_e(实体In));
        }
        else
        {
    ...
    private Collection<ScoreObjective> func_175137_e(实体 p_175137_1_)
    {
    ...
    public 实体Item dropPlayerItemWithRandomChoice(ItemStack itemStackIn)
    {
        return this.dropItem(itemStackIn, false, false);
    }

    public 实体Item dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem)
    {
        if (droppedItem == null)
        {
            return null;
        ...
        {
            double d0 = this.Y坐标 - 0.30000001192092896D + (double)this.getEyeHeight();
            实体Item entityitem = new 实体Item(this.worldObj, this.X坐标, d0, this.Z坐标, droppedItem);
            ...
            if (dropAround)
            {
                ...
                entityitem.通便X = -MathHelper.sin(f1) * f;
                entityitem.通便Z = MathHelper.cos(f1) * f;
                entityitem.motionY = 0.20000000298023224D;
            }
            else
            {
                float f2 = 0.3F;
                entityitem.通便X = -MathHelper.sin(this.旋转侧滑 / 180.0F * (float)Math.PI) * MathHelper.cos(this.rotationPitch / 180.0F * (float)Math.PI) * f2;
                entityitem.通便Z = MathHelper.cos(this.旋转侧滑 / 180.0F * (float)Math.PI) * MathHelper.cos(this.rotationPitch / 180.0F * (float)Math.PI) * f2;
                ...
                entityitem.通便X += Math.cos(f3) * (double)f2;
                entityitem.motionY += (this.rand.nextFloat() - this.rand.nextFloat()) * 0.1F;
                entityitem.通便Z += Math.sin(f3) * (double)f2;
            }
    ...
    public void readEntityFromNBT(NBTTagCompound tagCompund)
    {
        ...
        if (this.sleeping)
        {
            this.playerLocation = new 阻止位置(this);
            this.wakeUpPlayer(true, true, false);
        }

        if (tagCompund.hasKey("SpawnX", 99) && tagCompund.hasKey("SpawnY", 99) && tagCompund.hasKey("SpawnZ", 99))
        {
            this.spawnChunk = new 阻止位置(tagCompund.getInteger("SpawnX"), tagCompund.getInteger("SpawnY"), tagCompund.getInteger("SpawnZ"));
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (实体 instanceof 实体Arrow && ((实体Arrow) 实体).shooting实体 != null)
                    {
                        实体 = ((实体Arrow) 实体).shooting实体;
                    }
    ...
    public boolean canAttackPlayer(实体Player other)
    {
    ...
    public boolean interactWith(实体 target实体)
    {
        if (this.isSpectator())
        {
            if (target实体 instanceof IInventory)
            {
                this.displayGUIChest((IInventory) target实体);
            }
        ...
        else
        {
            ...
            if (!target实体.interactFirst(this))
            {
                if (itemstack != null && target实体 instanceof 实体LivingBase)
                {
                    ...
                    if (itemstack.interactWithEntity(this, (实体LivingBase) target实体))
                    {
    ...
    public void attackTargetEntityWithCurrentItem(实体 target实体)
    {
        if (target实体.canAttackWithItem())
        {
            if (!target实体.hitByEntity(this))
            {
                float f = (float)this.getEntityAttribute(SharedMonsterAttributes.attackDamage).getAttributeValue();
                int i = 0;
                float f1 = 0.0F;

                if (target实体 instanceof 实体LivingBase)
                {
                    f1 = EnchantmentHelper.getModifierForCreature(this.getHeldItem(), ((实体LivingBase) target实体).getCreatureAttribute());
                }
                else
                {
                ...
                if (f > 0.0F || f1 > 0.0F)
                {
                    boolean flag = this.fallDistance > 0.0F && !this.onGround && !this.isOnLadder() && !this.isInWater() && !this.isPotionActive(Potion.blindness) && this.riding实体 == null && target实体 instanceof 实体LivingBase;
                    ...
                    if (target实体 instanceof 实体LivingBase && j > 0 && !target实体.isBurning())
                    {
                        flag1 = true;
                        target实体.setFire(1);
                    }

                    double d0 = target实体.通便X;
                    double d1 = target实体.motionY;
                    double d2 = target实体.通便Z;
                    boolean flag2 = target实体.attackEntityFrom(DamageSource.causePlayerDamage(this), f);

                    if (flag2)
                    {
                        if (i > 0)
                        {
                            target实体.addVelocity(-MathHelper.sin(this.旋转侧滑 * (float)Math.PI / 180.0F) * (float)i * 0.5F, 0.1D, MathHelper.cos(this.旋转侧滑 * (float)Math.PI / 180.0F) * (float)i * 0.5F);
                            this.通便X *= 0.6D;
                            this.通便Z *= 0.6D;
                            this.设置宇轩的疾跑状态(false);
                        }

                        if (target实体 instanceof 实体PlayerMP && target实体.velocityChanged)
                        {
                            ((实体PlayerMP) target实体).playerNetServerHandler.sendPacket(new S12PacketEntityVelocity(target实体));
                            target实体.velocityChanged = false;
                            target实体.通便X = d0;
                            target实体.motionY = d1;
                            target实体.通便Z = d2;
                        }
                        ...
                        if (flag)
                        {
                            this.onCriticalHit(target实体);
                        }

                        if (f1 > 0.0F)
                        {
                            this.onEnchantmentCritical(target实体);
                        }
                        ...
                        this.setLastAttacker(target实体);

                        if (target实体 instanceof 实体LivingBase)
                        {
                            EnchantmentHelper.applyThornEnchantments((实体LivingBase) target实体, this);
                        }

                        EnchantmentHelper.applyArthropodEnchantments(this, target实体);
                        ItemStack itemstack = this.getCurrentEquippedItem();
                        实体 实体 = target实体;

                        if (target实体 instanceof 实体DragonPart)
                        {
                            IEntityMultiPart ientitymultipart = ((实体DragonPart) target实体).entityDragonObj;

                            if (ientitymultipart instanceof 实体LivingBase)
                            {
                                实体 = (实体LivingBase)ientitymultipart;
                            }
                        }

                        if (itemstack != null && 实体 instanceof 实体LivingBase)
                        {
                            itemstack.hitEntity((实体LivingBase) 实体, this);
    ...
    public 实体Player.EnumStatus trySleep(阻止位置 bedLocation)
    {
        if (!this.worldObj.isRemote)
        {
            if (this.isPlayerSleeping() || !this.isEntityAlive())
            {
                return 实体Player.EnumStatus.OTHER_PROBLEM;
            }

            if (!this.worldObj.provider.isSurfaceWorld())
            {
                return 实体Player.EnumStatus.NOT_POSSIBLE_HERE;
            }

            if (this.worldObj.isDaytime())
            {
                return 实体Player.EnumStatus.NOT_POSSIBLE_NOW;
            }

            if (Math.abs(this.X坐标 - (double)bedLocation.getX()) > 3.0D || Math.abs(this.Y坐标 - (double)bedLocation.getY()) > 2.0D || Math.abs(this.Z坐标 - (double)bedLocation.getZ()) > 3.0D)
            {
                return 实体Player.EnumStatus.TOO_FAR_AWAY;
            }

            double d0 = 8.0D;
            double d1 = 5.0D;
            List<实体Mob> list = this.worldObj.getEntitiesWithinAABB(实体Mob.class, new AxisAlignedBB((double)bedLocation.getX() - d0, (double)bedLocation.getY() - d1, (double)bedLocation.getZ() - d0, (double)bedLocation.getX() + d0, (double)bedLocation.getY() + d1, (double)bedLocation.getZ() + d0));

            if (!list.isEmpty())
            {
                return 实体Player.EnumStatus.NOT_SAFE;
            }
        }
        ...
        this.通便X = this.通便Z = this.motionY = 0.0D;
    ...
    public void wakeUpPlayer(boolean immediately, boolean updateWorldFlag, boolean setSpawn)
    {
        ...
        if (this.playerLocation != null && iblockstate.getBlock() == Blocks.bed)
        {
            ...
            阻止位置 blockpos = BlockBed.getSafeExitLocation(this.worldObj, this.playerLocation, 0);
    ...
    public static 阻止位置 getBedSpawnLocation(World worldIn, 阻止位置 bedLocation, boolean forceSpawn)
    {
    ...
    public void setSpawnPoint(阻止位置 pos, boolean forced)
    {
        if (pos != null)
        {
            this.spawnChunk = pos;
    ...
    public void moveEntityWithHeading(float strafe, float forward)
    {
        ...
        if (this.capabilities.isFlying && this.riding实体 == null)
        {
        ...
        this.addMovementStat(this.X坐标 - d0, this.Y坐标 - d1, this.Z坐标 - d2);
    }
    ...
    private void addMountedMovementStat(double p_71015_1_, double p_71015_3_, double p_71015_5_)
    {
        if (this.riding实体 != null)
        {
            ...
            if (i > 0)
            {
                if (this.riding实体 instanceof 实体Minecart)
                {
                    this.addStat(StatList.distanceByMinecartStat, i);

                    if (this.startMinecartRidingCoordinate == null)
                    {
                        this.startMinecartRidingCoordinate = new 阻止位置(this);
                    }
                    else if (this.startMinecartRidingCoordinate.distanceSq(MathHelper.floor_double(this.X坐标), MathHelper.floor_double(this.Y坐标), MathHelper.floor_double(this.Z坐标)) >= 1000000.0D)
                    {
                        this.triggerAchievement(AchievementList.onARail);
                    }
                }
                else if (this.riding实体 instanceof 实体Boat)
                {
                    this.addStat(StatList.distanceByBoatStat, i);
                }
                else if (this.riding实体 instanceof 实体Pig)
                {
                    this.addStat(StatList.distanceByPigStat, i);
                }
                else if (this.riding实体 instanceof 实体Horse)
                {
    ...
    public boolean canPlayerEdit(阻止位置 p_175151_1_, EnumFacing p_175151_2_, ItemStack p_175151_3_)
    {
        ...
        {
            阻止位置 blockpos = p_175151_1_.offset(p_175151_2_.getOpposite());
    ...
    public void clonePlayer(实体Player oldPlayer, boolean respawnFromEnd)
    {
        if (respawnFromEnd)
        {
    ...
    {
        IChatComponent ichatcomponent = new 交流组分文本(ScorePlayerTeam.formatPlayerName(this.getTeam(), this.getName()));
        ...
        else
        {
            ...
            if (i >= 0 && i < this.inventory.armorInventory.length)
            {
                ...
                if (itemStackIn != null && itemStackIn.getItem() != null)
                {
                    ...
                    {
                        if (实体Living.getArmorPosition(itemStackIn) != k)
                        {
                            return false;
                        }
                    }
    ...
    public enum EnumChatVisibility
    {
        ...
        private static final 实体Player.EnumChatVisibility[] ID_LOOKUP = new 实体Player.EnumChatVisibility[values().length];
        ...
        public static 实体Player.EnumChatVisibility getEnumChatVisibility(int id)
        {
        ...
        static {
            for (实体Player.EnumChatVisibility entityplayer$enumchatvisibility : values())
            {
