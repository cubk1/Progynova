...
public class 实体Wither extends 实体Mob implements IBossDisplayData, IRangedAttackMob
{
    ...
    private static final Predicate<实体> attackEntitySelector = new Predicate<实体>()
    {
        public boolean apply(实体 p_apply_1_)
        {
            return p_apply_1_ instanceof 实体LivingBase && ((实体LivingBase)p_apply_1_).getCreatureAttribute() != EnumCreatureAttribute.UNDEAD;
        }
    };

    public 实体Wither(World worldIn)
    {
        super(worldIn);
        ...
        this.targetTasks.addTask(2, new EntityAINearestAttackableTarget(this, 实体Living.class, 0, false, false, attackEntitySelector));
    ...
    public void onLivingUpdate()
    {
        ...
        {
            实体 实体 = this.worldObj.getEntityByID(this.getWatchedTargetId(0));

            if (实体 != null)
            {
                if (this.Y坐标 < 实体.Y坐标 || !this.isArmored() && this.Y坐标 < 实体.Y坐标 + 5.0D)
                {
                ...
                double d0 = 实体.X坐标 - this.X坐标;
                double d1 = 实体.Z坐标 - this.Z坐标;
                double d3 = d0 * d0 + d1 * d1;

                if (d3 > 9.0D)
                {
                    double d5 = (double)MathHelper.sqrt_double(d3);
                    this.通便X += (d0 / d5 * 0.5D - this.通便X) * 0.6000000238418579D;
                    this.通便Z += (d1 / d5 * 0.5D - this.通便Z) * 0.6000000238418579D;
                }
            }
        }

        if (this.通便X * this.通便X + this.通便Z * this.通便Z > 0.05000000074505806D)
        {
            this.旋转侧滑 = (float)MathHelper.atan2(this.通便Z, this.通便X) * (180F / (float)Math.PI) - 90.0F;
        }
        ...
        for (int j = 0; j < 2; ++j)
        {
            ...
            if (k > 0)
            {
                实体1 = this.worldObj.getEntityByID(k);
            }
            ...
            if (实体1 != null)
            {
                ...
                double d6 = 实体1.X坐标 - d11;
                double d7 = 实体1.Y坐标 + (double) 实体1.getEyeHeight() - d12;
                double d8 = 实体1.Z坐标 - d13;
        ...
        if (this.getInvulTime() > 0)
        {
            ...
            {
                this.worldObj.spawnParticle(EnumParticleTypes.SPELL_MOB, this.X坐标 + this.rand.nextGaussian() * 1.0D, this.Y坐标 + (double)(this.rand.nextFloat() * 3.3F), this.Z坐标 + this.rand.nextGaussian() * 1.0D, 0.699999988079071D, 0.699999988079071D, 0.8999999761581421D, new int[0]);
            }
        }
    }
    ...
    protected void updateAITasks()
    {
        if (this.getInvulTime() > 0)
        {
            ...
            if (j1 <= 0)
            {
                this.worldObj.newExplosion(this, this.X坐标, this.Y坐标 + (double)this.getEyeHeight(), this.Z坐标, 7.0F, false, this.worldObj.getGameRules().getBoolean("mobGriefing"));
                this.worldObj.playBroadcastSound(1013, new 阻止位置(this), 0);
            }
        ...
        else
        {
                ...
                if (this.已存在的刻度 >= this.field_82223_h[i - 1])
                {
                    ...
                    if (this.worldObj.getDifficulty() == EnumDifficulty.NORMAL || this.worldObj.getDifficulty() == EnumDifficulty.HARD)
                    {
                        ...
                        if (k3 > 15)
                        {
                            float f = 10.0F;
                            float f1 = 5.0F;
                            double d0 = MathHelper.getRandomDoubleInRange(this.rand, this.X坐标 - (double)f, this.X坐标 + (double)f);
                            double d1 = MathHelper.getRandomDoubleInRange(this.rand, this.Y坐标 - (double)f1, this.Y坐标 + (double)f1);
                            double d2 = MathHelper.getRandomDoubleInRange(this.rand, this.Z坐标 - (double)f, this.Z坐标 + (double)f);
                    ...
                    if (k1 > 0)
                    {
                        实体 实体 = this.worldObj.getEntityByID(k1);

                        if (实体 != null && 实体.isEntityAlive() && this.getDistanceSqToEntity(实体) <= 900.0D && this.canEntityBeSeen(实体))
                        {
                            ...
                            {
                                this.launchWitherSkullToEntity(i + 1, (实体LivingBase) 实体);
                    ...
                    {
                        List<实体LivingBase> list = this.worldObj.<实体LivingBase>getEntitiesWithinAABB(实体LivingBase.class, this.getEntityBoundingBox().expand(20.0D, 8.0D, 20.0D), Predicates.<实体LivingBase> and (attackEntitySelector, EntitySelectors.NOT_SPECTATING));

                        for (int j2 = 0; j2 < 10 && !list.isEmpty(); ++j2)
                        {
                            实体LivingBase entitylivingbase = (实体LivingBase)list.get(this.rand.nextInt(list.size()));

                            if (entitylivingbase != this && entitylivingbase.isEntityAlive() && this.canEntityBeSeen(entitylivingbase))
                            {
                                if (entitylivingbase instanceof 实体Player)
                                {
                                    if (!((实体Player)entitylivingbase).capabilities.disableDamage)
                                    {
            ...
            if (this.blockBreakCounter > 0)
            {
                ...
                {
                    int i1 = MathHelper.floor_double(this.Y坐标);
                    int l1 = MathHelper.floor_double(this.X坐标);
                    int i2 = MathHelper.floor_double(this.Z坐标);
                    boolean flag = false;
                    ...
                    if (flag)
                    {
                        this.worldObj.playAuxSFXAtEntity((实体Player)null, 1012, new 阻止位置(this), 0);
                    }
                }
            }
    ...
    private double func_82214_u(int p_82214_1_)
    {
        ...
        else
        {
            ...
            return this.X坐标 + (double)f1 * 1.3D;
        }
    }

    private double func_82208_v(int p_82208_1_)
    {
        return p_82208_1_ <= 0 ? this.Y坐标 + 3.0D : this.Y坐标 + 2.2D;
    }
    ...
    private double func_82213_w(int p_82213_1_)
    {
        ...
        else
        {
            ...
            return this.Z坐标 + (double)f1 * 1.3D;
        }
    }
    ...
    private void launchWitherSkullToEntity(int p_82216_1_, 实体LivingBase p_82216_2_)
    {
        this.launchWitherSkullToCoords(p_82216_1_, p_82216_2_.X坐标, p_82216_2_.Y坐标 + (double)p_82216_2_.getEyeHeight() * 0.5D, p_82216_2_.Z坐标, p_82216_1_ == 0 && this.rand.nextFloat() < 0.001F);
    }

    private void launchWitherSkullToCoords(int p_82209_1_, double x, double y, double z, boolean invulnerable)
    {
        this.worldObj.playAuxSFXAtEntity((实体Player)null, 1014, new 阻止位置(this), 0);
        ...
        double d4 = y - d1;
        double d5 = z - d2;
        实体WitherSkull entitywitherskull = new 实体WitherSkull(this.worldObj, this, d3, d4, d5);
        ...
        entitywitherskull.Y坐标 = d1;
        entitywitherskull.X坐标 = d0;
        entitywitherskull.Z坐标 = d2;
    ...
    public boolean attackEntityFrom(DamageSource source, float amount)
    {
        ...
        else if (source != DamageSource.drown && !(source.getEntity() instanceof 实体Wither))
        {
            ...
            else
            {
                ...
                实体 实体1 = source.getEntity();

                if (实体1 != null && !(实体1 instanceof 实体Player) && 实体1 instanceof 实体LivingBase && ((实体LivingBase) 实体1).getCreatureAttribute() == this.getCreatureAttribute())
                {
                    return false;
                }
                else
    ...
    {
        实体Item entityitem = this.dropItem(Items.nether_star, 1);
        ...
        {
            for (实体Player entityplayer : this.worldObj.getEntitiesWithinAABB(实体Player.class, this.getEntityBoundingBox().expand(50.0D, 100.0D, 50.0D)))
            {
