...
public abstract class 实体Ageable extends 实体Creature
{
    ...
    public 实体Ageable(World worldIn)
    {
        super(worldIn);
    }

    public abstract 实体Ageable createChild(实体Ageable ageable);
    ...
    public boolean interact(实体Player player)
    {
        ...
        if (itemstack != null && itemstack.getItem() == Items.spawn_egg)
        {
            ...
            {
                Class <? extends 实体> oclass = EntityList.getClassFromID(itemstack.getMetadata());

                if (oclass != null && this.getClass() == oclass)
                {
                    实体Ageable entityageable = this.createChild(this);

                    if (entityageable != null)
                    {
                        entityageable.setGrowingAge(-24000);
                        entityageable.setLocationAndAngles(this.X坐标, this.Y坐标, this.Z坐标, 0.0F, 0.0F);
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.worldObj.isRemote)
        {
            if (this.field_175503_c > 0)
            {
                ...
                {
                    this.worldObj.spawnParticle(EnumParticleTypes.VILLAGER_HAPPY, this.X坐标 + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, this.Y坐标 + 0.5D + (double)(this.rand.nextFloat() * this.height), this.Z坐标 + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, 0.0D, 0.0D, 0.0D, new int[0]);
                }
