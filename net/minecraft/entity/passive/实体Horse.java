...
public class 实体Horse extends 实体Animal implements IInvBasic
{
    private static final Predicate<实体> horseBreedingSelector = new Predicate<实体>()
    {
        public boolean apply(实体 p_apply_1_)
        {
            return p_apply_1_ instanceof 实体Horse && ((实体Horse)p_apply_1_).isBreeding();
        }
    };
    ...
    public boolean attackEntityFrom(DamageSource source, float amount)
    {
        ...
        return this.riddenBy实体 != null && this.riddenBy实体.equals(实体) ? false : super.attackEntityFrom(source, amount);
    }
    ...
    {
        return this.riddenBy实体 == null;
    }

    public boolean prepareChunkForSpawn()
    {
        int i = MathHelper.floor_double(this.X坐标);
        int j = MathHelper.floor_double(this.Z坐标);
        this.worldObj.getBiomeGenForCoords(new 阻止位置(i, 0, j));
        return true;
    }
    ...
    public void fall(float distance, float damageMultiplier)
    {
        ...
        if (i > 0)
        {
            ...
            {
                this.riddenBy实体.attackEntityFrom(DamageSource.fall, (float)i);
            }

            Block block = this.worldObj.getBlockState(new 阻止位置(this.X坐标, this.Y坐标 - 0.2D - (double)this.prevRotationYaw, this.Z坐标)).getBlock();
    ...
    protected 实体Horse getClosestHorse(实体 实体In, double distance)
    {
        double d0 = Double.MAX_VALUE;
        实体 实体 = null;

        for (实体 实体1 : this.worldObj.getEntitiesInAABBexcluding(实体In, 实体In.getEntityBoundingBox().addCoord(distance, distance, distance), horseBreedingSelector))
        {
            double d1 = 实体1.getDistanceSq(实体In.X坐标, 实体In.Y坐标, 实体In.Z坐标);
    ...
    protected void playStepSound(阻止位置 pos, Block blockIn)
    {
        ...
        if (!blockIn.getMaterial().isLiquid())
        {
            ...
            if (this.riddenBy实体 != null && i != 1 && i != 2)
            {
                ++this.gallopTime;
    ...
    public void openGUI(实体Player playerEntity)
    {
        if (!this.worldObj.isRemote && (this.riddenBy实体 == null || this.riddenBy实体 == playerEntity) && this.isTame())
        {
            this.horseChest.setCustomName(this.getName());
    ...
    public boolean interact(实体Player player)
    {
        ...
        else if (this.isTame() && this.isAdultHorse() && player.正在下蹲())
        {
            this.openGUI(player);
            return true;
        }
        else if (this.func_110253_bW() && this.riddenBy实体 != null)
        {
            return super.interact(player);
        }
        else
        {
            ...
            if (this.func_110253_bW() && this.riddenBy实体 == null)
            {
                if (itemstack != null && itemstack.interactWithEntity(player, this))
                {
                    return true;
    ...
    private void mountTo(实体Player player)
    {
        player.旋转侧滑 = this.旋转侧滑;
    ...
    {
        return this.riddenBy实体 != null && this.isHorseSaddled() ? true : this.isEatingHaystack() || this.isRearing();
    }
    ...
    public void onLivingUpdate()
    {
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (!this.isEatingHaystack() && this.riddenBy实体 == null && this.rand.nextInt(300) == 0 && this.worldObj.getBlockState(new 阻止位置(MathHelper.floor_double(this.X坐标), MathHelper.floor_double(this.Y坐标) - 1, MathHelper.floor_double(this.Z坐标))).getBlock() == Blocks.grass)
            {
            ...
            {
                实体Horse entityhorse = this.getClosestHorse(this, 16.0D);
    ...
    {
        return this.riddenBy实体 == null && this.riding实体 == null && this.isTame() && this.isAdultHorse() && !this.isSterile() && this.getHealth() >= this.getMaxHealth() && this.isInLove();
    }
    ...
    private void dropItemsInChest(实体 实体In, AnimalChest animalChestIn)
    {
    ...
    public boolean setTamedBy(实体Player player)
    {
    ...
    {
        if (this.riddenBy实体 != null && this.riddenBy实体 instanceof 实体LivingBase && this.isHorseSaddled())
        {
            this.prevRotationYaw = this.旋转侧滑 = this.riddenBy实体.旋转侧滑;
            this.rotationPitch = this.riddenBy实体.rotationPitch * 0.5F;
            this.setRotation(this.旋转侧滑, this.rotationPitch);
            this.rotationYawHead = this.renderYawOffset = this.旋转侧滑;
            strafe = ((实体LivingBase)this.riddenBy实体).moveStrafing * 0.5F;
            forward = ((实体LivingBase)this.riddenBy实体).moveForward;
            ...
            if (this.jumpPower > 0.0F && !this.isHorseJumping() && this.onGround)
            {
                ...
                if (forward > 0.0F)
                {
                    float f = MathHelper.sin(this.旋转侧滑 * (float)Math.PI / 180.0F);
                    float f1 = MathHelper.cos(this.旋转侧滑 * (float)Math.PI / 180.0F);
                    this.通便X += (double)(-0.4F * f * this.jumpPower);
                    this.通便Z += (double)(0.4F * f1 * this.jumpPower);
    ...
    public boolean canMateWith(实体Animal otherAnimal)
    {
        ...
        {
            实体Horse entityhorse = (实体Horse)otherAnimal;
    ...
    public 实体Ageable createChild(实体Ageable ageable)
    {
        实体Horse entityhorse = (实体Horse)ageable;
        实体Horse entityhorse1 = new 实体Horse(this.worldObj);
        int i = this.getHorseType();
        int j = entityhorse.getHorseType();
        int k = 0;
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData livingdata)
    {
        ...
        if (livingdata instanceof 实体Horse.GroupData)
        {
            i = ((实体Horse.GroupData)livingdata).horseType;
            j = ((实体Horse.GroupData)livingdata).horseVariant & 255 | this.rand.nextInt(5) << 8;
        }
        else
        {
            ...
            livingdata = new 实体Horse.GroupData(i, j);
        }
    ...
    public void updateRiderPosition()
    {
        ...
        if (this.prevRearingAmount > 0.0F)
        {
            ...
            this.riddenBy实体.setPosition(this.X坐标 + (double)(f2 * f), this.Y坐标 + this.getMountedYOffset() + this.riddenBy实体.getYOffset() + (double)f3, this.Z坐标 - (double)(f2 * f1));
