...
public class 实体Sheep extends 实体Animal
{
    ...
    public boolean interact(实体Player player)
    {
        ...
        if (itemstack != null && itemstack.getItem() == Items.shears && !this.getSheared() && !this.isChild())
        {
            if (!this.worldObj.isRemote)
            {
                ...
                for (int j = 0; j < i; ++j)
                {
                    实体Item entityitem = this.entityDropItem(new ItemStack(Item.getItemFromBlock(Blocks.wool), 1, this.getFleeceColor().getMetadata()), 1.0F);
                    entityitem.motionY += (double)(this.rand.nextFloat() * 0.05F);
                    entityitem.通便X += (double)((this.rand.nextFloat() - this.rand.nextFloat()) * 0.1F);
                    entityitem.通便Z += (double)((this.rand.nextFloat() - this.rand.nextFloat()) * 0.1F);
                }
            }
    ...
    public 实体Sheep createChild(实体Ageable ageable)
    {
        实体Sheep entitysheep = (实体Sheep)ageable;
        实体Sheep entitysheep1 = new 实体Sheep(this.worldObj);
    ...
    private EnumDyeColor getDyeColorMixFromParents(实体Animal father, 实体Animal mother)
    {
        int i = ((实体Sheep)father).getFleeceColor().getDyeDamage();
        int j = ((实体Sheep)mother).getFleeceColor().getDyeDamage();
        this.inventoryCrafting.getStackInSlot(0).setItemDamage(i);
        this.inventoryCrafting.getStackInSlot(1).setItemDamage(j);
        ItemStack itemstack = CraftingManager.getInstance().findMatchingRecipe(this.inventoryCrafting, ((实体Sheep)father).worldObj);
        int k;
