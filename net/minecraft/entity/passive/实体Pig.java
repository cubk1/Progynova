...
public class 实体Pig extends 实体Animal
{
    private final EntityAIControlledByPlayer aiControlledByPlayer;

    public 实体Pig(World worldIn)
    {
        super(worldIn);
    ...
    {
        ItemStack itemstack = ((实体Player)this.riddenBy实体).getHeldItem();
    ...
    public boolean interact(实体Player player)
    {
        ...
        else if (!this.getSaddled() || this.worldObj.isRemote || this.riddenBy实体 != null && this.riddenBy实体 != player)
        {
            return false;
        }
        else
    ...
    public void onStruckByLightning(实体LightningBolt lightningBolt)
    {
        ...
        {
            实体PigZombie entitypigzombie = new 实体PigZombie(this.worldObj);
            entitypigzombie.setCurrentItemOrArmor(0, new ItemStack(Items.golden_sword));
            entitypigzombie.setLocationAndAngles(this.X坐标, this.Y坐标, this.Z坐标, this.旋转侧滑, this.rotationPitch);
    ...
    public void fall(float distance, float damageMultiplier)
    {
        ...
        if (distance > 5.0F && this.riddenBy实体 instanceof 实体Player)
        {
            ((实体Player)this.riddenBy实体).triggerAchievement(AchievementList.flyPig);
        }
    }

    public 实体Pig createChild(实体Ageable ageable)
    {
        return new 实体Pig(this.worldObj);
    }
