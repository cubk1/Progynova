...
public class 实体Rabbit extends 实体Animal
{
    private 实体Rabbit.AIAvoidEntity<实体Wolf> aiAvoidWolves;
    ...
    private 实体Rabbit.EnumMoveType moveType = 实体Rabbit.EnumMoveType.HOP;
    private int carrotTicks = 0;
    private 实体Player field_175543_bt = null;

    public 实体Rabbit(World worldIn)
    {
        super(worldIn);
        this.setSize(0.6F, 0.7F);
        this.jumpHelper = new 实体Rabbit.RabbitJumpHelper(this);
        this.moveHelper = new 实体Rabbit.RabbitMoveHelper(this);
        ((PathNavigateGround)this.getNavigator()).setAvoidsWater(true);
        this.navigator.setHeightRequirement(2.5F);
        this.tasks.addTask(1, new EntityAISwimming(this));
        this.tasks.addTask(1, new 实体Rabbit.AIPanic(this, 1.33D));
        ...
        this.tasks.addTask(5, new 实体Rabbit.AIRaidFarm(this));
        this.tasks.addTask(5, new EntityAIWander(this, 0.6D));
        this.tasks.addTask(11, new EntityAIWatchClosest(this, 实体Player.class, 10.0F));
        this.aiAvoidWolves = new 实体Rabbit.AIAvoidEntity(this, 实体Wolf.class, 16.0F, 1.33D, 1.33D);
    ...
    {
        return this.moveHelper.isUpdating() && this.moveHelper.getY() > this.Y坐标 + 0.5D ? 0.5F : this.moveType.func_180074_b();
    }

    public void setMoveType(实体Rabbit.EnumMoveType type)
    {
        this.moveType = type;
    }
    ...
    public void setJumping(boolean jump, 实体Rabbit.EnumMoveType moveTypeIn)
    {
        super.setJumping(jump);

        if (!jump)
        {
            if (this.moveType == 实体Rabbit.EnumMoveType.ATTACK)
            {
                this.moveType = 实体Rabbit.EnumMoveType.HOP;
            }
        }
        else
        {
    ...
    public void doMovementAction(实体Rabbit.EnumMoveType movetype)
    {
    ...
    public void updateAITasks()
    {
        ...
        {
            this.setMoveType(实体Rabbit.EnumMoveType.SPRINT);
        }
        else if (this.moveType != 实体Rabbit.EnumMoveType.ATTACK)
        {
            this.setMoveType(实体Rabbit.EnumMoveType.HOP);
        }
        ...
        if (this.onGround)
        {
            ...
            {
                this.setJumping(false, 实体Rabbit.EnumMoveType.NONE);
                this.func_175517_cu();
            }
            ...
            if (this.getRabbitType() == 99 && this.currentMoveTypeDuration == 0)
            {
                ...
                {
                    this.calculateRotationYaw(entitylivingbase.X坐标, entitylivingbase.Z坐标);
                    this.moveHelper.setMoveTo(entitylivingbase.X坐标, entitylivingbase.Y坐标, entitylivingbase.Z坐标, this.moveHelper.getSpeed());
                    this.doMovementAction(实体Rabbit.EnumMoveType.ATTACK);
                    this.field_175537_bp = true;
                }
            }

            实体Rabbit.RabbitJumpHelper entityrabbit$rabbitjumphelper = (实体Rabbit.RabbitJumpHelper)this.jumpHelper;
    ...
    {
        this.旋转侧滑 = (float)(MathHelper.atan2(z - this.Z坐标, x - this.X坐标) * 180.0D / Math.PI) - 90.0F;
    }

    private void func_175518_cr()
    {
        ((实体Rabbit.RabbitJumpHelper)this.jumpHelper).func_180066_a(true);
    }

    private void func_175520_cs()
    {
        ((实体Rabbit.RabbitJumpHelper)this.jumpHelper).func_180066_a(false);
    }
    ...
    public boolean attackEntityAsMob(实体 实体In)
    {
        if (this.getRabbitType() == 99)
        {
            ...
            return 实体In.attackEntityFrom(DamageSource.causeMobDamage(this), 8.0F);
        }
        else
        {
    ...
    public 实体Rabbit createChild(实体Ageable ageable)
    {
        实体Rabbit entityrabbit = new 实体Rabbit(this.worldObj);

        if (ageable instanceof 实体Rabbit)
        {
            entityrabbit.setRabbitType(this.rand.nextBoolean() ? this.getRabbitType() : ((实体Rabbit)ageable).getRabbitType());
        }
    ...
    public void setRabbitType(int rabbitTypeId)
    {
        if (rabbitTypeId == 99)
        {
            ...
            this.tasks.addTask(4, new 实体Rabbit.AIEvilAttack(this));
            this.targetTasks.addTask(1, new EntityAIHurtByTarget(this, false, new Class[0]));
            this.targetTasks.addTask(2, new EntityAINearestAttackableTarget(this, 实体Player.class, true));
            this.targetTasks.addTask(2, new EntityAINearestAttackableTarget(this, 实体Wolf.class, true));
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData livingdata)
    {
        ...
        if (livingdata instanceof 实体Rabbit.RabbitTypeData)
        {
            i = ((实体Rabbit.RabbitTypeData)livingdata).typeData;
            flag = true;
        }
        else
        {
            livingdata = new 实体Rabbit.RabbitTypeData(i);
        }
    ...
    {
        this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_DUST, this.X坐标 + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, this.Y坐标 + 0.5D + (double)(this.rand.nextFloat() * this.height), this.Z坐标 + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, 0.0D, 0.0D, 0.0D, new int[] {Block.getStateId(Blocks.carrots.getStateFromMeta(7))});
    ...
    static class AIAvoidEntity<T extends 实体> extends EntityAIAvoidEntity<T>
    {
        private 实体Rabbit entityInstance;

        public AIAvoidEntity(实体Rabbit rabbit, Class<T> p_i46403_2_, float p_i46403_3_, double p_i46403_4_, double p_i46403_6_)
        {
    ...
    {
        public AIEvilAttack(实体Rabbit rabbit)
        {
            super(rabbit, 实体LivingBase.class, 1.4D, true);
        }
    ...
    {
        private 实体Rabbit theEntity;

        public AIPanic(实体Rabbit rabbit, double speedIn)
        {
    ...
    public class RabbitJumpHelper extends EntityJumpHelper
    {
        ...
        public RabbitJumpHelper(实体Rabbit rabbit)
        {
            super(rabbit);
            this.theEntity = rabbit;
        }
        ...
        public void doJump()
        {
            if (this.isJumping)
            {
                this.theEntity.doMovementAction(实体Rabbit.EnumMoveType.STEP);
    ...
    static class RabbitMoveHelper extends EntityMoveHelper
    {
        ...
        public RabbitMoveHelper(实体Rabbit rabbit)
        {
            super(rabbit);
            this.theEntity = rabbit;
        }
