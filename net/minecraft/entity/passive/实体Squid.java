...
public class 实体Squid extends 实体WaterMob
{
    ...
    public 实体Squid(World worldIn)
    {
        super(worldIn);
        this.setSize(0.95F, 0.95F);
        this.rand.setSeed((long)(1 + this.getEntityId()));
        this.rotationVelocity = 1.0F / (this.rand.nextFloat() + 1.0F) * 0.2F;
        this.tasks.addTask(0, new 实体Squid.AIMoveRandom(this));
    }
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.inWater)
        {
            ...
            if (!this.worldObj.isRemote)
            {
                this.通便X = (double)(this.randomMotionVecX * this.randomMotionSpeed);
                this.motionY = (double)(this.randomMotionVecY * this.randomMotionSpeed);
                this.通便Z = (double)(this.randomMotionVecZ * this.randomMotionSpeed);
            }

            float f1 = MathHelper.sqrt_double(this.通便X * this.通便X + this.通便Z * this.通便Z);
            this.renderYawOffset += (-((float)MathHelper.atan2(this.通便X, this.通便Z)) * 180.0F / (float)Math.PI - this.renderYawOffset) * 0.1F;
            this.旋转侧滑 = this.renderYawOffset;
    ...
    {
        return this.Y坐标 > 45.0D && this.Y坐标 < (double)this.worldObj.getSeaLevel() && super.getCanSpawnHere();
    }
    ...
    {
        private 实体Squid squid;

        public AIMoveRandom(实体Squid p_i45859_1_)
        {
