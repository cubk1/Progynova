...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            double d4 = y;
            double d5 = z;
            boolean flag = this.onGround && this.正在下蹲() && this instanceof EntityPlayer;
    ...
    protected void resetHeight()
    {
        ...
        for (int i = 0; (float)i < 1.0F + this.width * 20.0F; ++i)
        {
            ...
            this.worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + (double)f2, (double)(f1 + 1.0F), this.posZ + (double)f3, this.通便X, this.motionY - (double)(this.rand.nextFloat() * 0.2F), this.通便Z, new int[0]);
        }
        ...
        for (int j = 0; (float)j < 1.0F + this.width * 20.0F; ++j)
        {
            ...
            this.worldObj.spawnParticle(EnumParticleTypes.WATER_SPLASH, this.posX + (double)f4, (double)(f1 + 1.0F), this.posZ + (double)f5, this.通便X, this.motionY, this.通便Z, new int[0]);
        }
    }
    ...
    protected void createRunningParticles()
    {
        ...
        {
            this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + ((double)this.rand.nextFloat() - 0.5D) * (double)this.width, this.getEntityBoundingBox().minY + 0.1D, this.posZ + ((double)this.rand.nextFloat() - 0.5D) * (double)this.width, -this.通便X * 4.0D, 1.5D, -this.通便Z * 4.0D, new int[] {Block.getStateId(iblockstate)});
        }
    }
    ...
    public void updateRidden()
    {
        ...
        else
        {
            ...
            if (this.ridingEntity != null)
            {
                ...
                this.entityRiderYawDelta += (double)(this.ridingEntity.旋转侧滑 - this.ridingEntity.prevRotationYaw);
    ...
    public void mountEntity(Entity entityIn)
    {
        ...
        if (entityIn == null)
        {
            if (this.ridingEntity != null)
            {
                this.setLocationAndAngles(this.ridingEntity.posX, this.ridingEntity.getEntityBoundingBox().minY + (double)this.ridingEntity.height, this.ridingEntity.posZ, this.旋转侧滑, this.rotationPitch);
    ...
    {
        this.setLocationAndAngles(entityIn.posX, entityIn.posY, entityIn.posZ, entityIn.旋转侧滑, entityIn.rotationPitch);
    }
        ...
        if (!this.worldObj.isRemote && !this.isDead)
        {
            ...
            if (entity != null)
            {
                ...
                if (i == 1 && dimensionId == 1)
                {
                    ...
                    entity.moveToBlockPosAndAngles(blockpos, entity.旋转侧滑, entity.rotationPitch);
                }
