...
public class EntityArrow extends Entity implements IProjectile
{
    ...
    public EntityArrow(World worldIn, EntityLivingBase shooter, float velocity)
    {
        ...
        this.setSize(0.5F, 0.5F);
        this.setLocationAndAngles(shooter.posX, shooter.posY + (double)shooter.getEyeHeight(), shooter.posZ, shooter.旋转侧滑, shooter.rotationPitch);
    ...
    public void setVelocity(double x, double y, double z)
    {
        ...
        if (this.prevRotationPitch == 0.0F && this.prevRotationYaw == 0.0F)
        {
            ...
            this.setLocationAndAngles(this.posX, this.posY, this.posZ, this.旋转侧滑, this.rotationPitch);
            this.ticksInGround = 0;
        }
    }
    ...
    public void onUpdate()
    {
        ...
        else
        {
            ...
            Vec3 vec3 = new Vec3(this.posX + this.通便X, this.posY + this.motionY, this.posZ + this.通便Z);
            MovingObjectPosition movingobjectposition = this.worldObj.rayTraceBlocks(vec31, vec3, false, true, false);
            vec31 = new Vec3(this.posX, this.posY, this.posZ);
            vec3 = new Vec3(this.posX + this.通便X, this.posY + this.motionY, this.posZ + this.通便Z);
            ...
            if (movingobjectposition != null)
            {
                    ...
                    if (movingobjectposition.entityHit.attackEntityFrom(damagesource, (float)l))
                    {
                        if (movingobjectposition.entityHit instanceof EntityLivingBase)
                        {
                            ...
                            if (this.knockbackStrength > 0)
                            {
                                ...
                                if (f7 > 0.0F)
                                {
                                    movingobjectposition.entityHit.addVelocity(this.通便X * (double)this.knockbackStrength * 0.6000000238418579D / (double)f7, 0.1D, this.通便Z * (double)this.knockbackStrength * 0.6000000238418579D / (double)f7);
                                }
                            }
                ...
                else
                {
                    ...
                    this.通便X = (double)((float)(movingobjectposition.hitVec.xCoord - this.posX));
                    this.motionY = (double)((float)(movingobjectposition.hitVec.yCoord - this.posY));
                    this.通便Z = (double)((float)(movingobjectposition.hitVec.zCoord - this.posZ));
                    float f5 = MathHelper.sqrt_double(this.通便X * this.通便X + this.motionY * this.motionY + this.通便Z * this.通便Z);
                    this.posX -= this.通便X / (double)f5 * 0.05000000074505806D;
                    this.posY -= this.motionY / (double)f5 * 0.05000000074505806D;
                    this.posZ -= this.通便Z / (double)f5 * 0.05000000074505806D;
                    this.playSound("random.bowhit", 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
                    this.inGround = true;
                    this.arrowShake = 7;
            ...
            if (this.getIsCritical())
            {
                for (int k = 0; k < 4; ++k)
                {
                    this.worldObj.spawnParticle(EnumParticleTypes.CRIT, this.posX + this.通便X * (double)k / 4.0D, this.posY + this.motionY * (double)k / 4.0D, this.posZ + this.通便Z * (double)k / 4.0D, -this.通便X, -this.motionY + 0.2D, -this.通便Z, new int[0]);
                }
            }
            ...
            if (this.isInWater())
            {
                ...
                {
                    float f8 = 0.25F;
                    this.worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX - this.通便X * (double)f8, this.posY - this.motionY * (double)f8, this.posZ - this.通便Z * (double)f8, this.通便X, this.motionY, this.通便Z, new int[0]);
                }
