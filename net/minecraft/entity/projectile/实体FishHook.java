...
public class 实体FishHook extends 实体
{
    ...
    public 实体FishHook(World worldIn)
    {
        super(worldIn);
        this.xTile = -1;
        this.yTile = -1;
    ...
    public 实体FishHook(World worldIn, double x, double y, double z, 实体Player anglerIn)
    {
        this(worldIn);
    ...
    public 实体FishHook(World worldIn, 实体Player fishingPlayer)
    {
        super(worldIn);
        this.xTile = -1;
        this.yTile = -1;
        ...
        this.setSize(0.25F, 0.25F);
        this.setLocationAndAngles(fishingPlayer.X坐标, fishingPlayer.Y坐标 + (double)fishingPlayer.getEyeHeight(), fishingPlayer.Z坐标, fishingPlayer.旋转侧滑, fishingPlayer.rotationPitch);
        ...
        this.handleHookCasting(this.通便X, this.motionY, this.通便Z, 1.5F, 1.0F);
    }
    ...
    public void handleHookCasting(double p_146035_1_, double p_146035_3_, double p_146035_5_, float p_146035_7_, float p_146035_8_)
    {
        ...
        this.通便X = p_146035_1_;
        this.motionY = p_146035_3_;
        this.通便Z = p_146035_5_;
        float f1 = MathHelper.sqrt_double(p_146035_1_ * p_146035_1_ + p_146035_5_ * p_146035_5_);
        this.prevRotationYaw = this.旋转侧滑 = (float)(MathHelper.atan2(p_146035_1_, p_146035_5_) * 180.0D / Math.PI);
    ...
    public void setPositionAndRotation2(double x, double y, double z, float yaw, float pitch, int posRotationIncrements, boolean p_180426_10_)
    {
        ...
        this.通便X = this.clientMotionX;
        this.motionY = this.clientMotionY;
        this.通便Z = this.clientMotionZ;
    }

    public void setVelocity(double x, double y, double z)
    {
        this.clientMotionX = this.通便X = x;
        this.clientMotionY = this.motionY = y;
        this.clientMotionZ = this.通便Z = z;
    }
    ...
    public void onUpdate()
    {
        ...
        {
            double d7 = this.X坐标 + (this.fishX - this.X坐标) / (double)this.fishPosRotationIncrements;
            double d8 = this.Y坐标 + (this.fishY - this.Y坐标) / (double)this.fishPosRotationIncrements;
            double d9 = this.Z坐标 + (this.fishZ - this.Z坐标) / (double)this.fishPosRotationIncrements;
            double d1 = MathHelper.wrapAngleTo180_double(this.fishYaw - (double)this.旋转侧滑);
            this.旋转侧滑 = (float)((double)this.旋转侧滑 + d1 / (double)this.fishPosRotationIncrements);
        ...
        else
        {
            if (!this.worldObj.isRemote)
            {
                ...
                if (this.caught实体 != null)
                {
                    if (!this.caught实体.isDead)
                    {
                        this.X坐标 = this.caught实体.X坐标;
                        double d17 = (double)this.caught实体.height;
                        this.Y坐标 = this.caught实体.getEntityBoundingBox().minY + d17 * 0.8D;
                        this.Z坐标 = this.caught实体.Z坐标;
                        return;
                    }
            ...
            for (int i = 0; i < list.size(); ++i)
            {
                ...
                if (实体1.canBeCollidedWith() && (实体1 != this.angler || this.ticksInAir >= 5))
                {
                    float f = 0.3F;
            ...
            if (movingobjectposition != null)
            {
                ...
                {
                    if (movingobjectposition.实体Hit.attackEntityFrom(DamageSource.causeThrownDamage(this, this.angler), 0.0F))
                    {
                        this.caught实体 = movingobjectposition.实体Hit;
                    }
                }
                else
                {
            ...
            if (!this.inGround)
            {
                ...
                float f5 = MathHelper.sqrt_double(this.通便X * this.通便X + this.通便Z * this.通便Z);
                ...
                if (!this.worldObj.isRemote && d10 > 0.0D)
                {
                    ...
                    int l = 1;
                    阻止位置 blockpos = (new 阻止位置(this)).up();
                    ...
                    else if (this.ticksCatchableDelay > 0)
                    {
                        ...
                        if (this.ticksCatchableDelay <= 0)
                        {
                            ...
                            worldserver.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.X坐标, (double)(f8 + 1.0F), this.Z坐标, (int)(1.0F + this.width * 20.0F), (double)this.width, 0.0D, (double)this.width, 0.20000000298023224D, new int[0]);
                            worldserver.spawnParticle(EnumParticleTypes.WATER_WAKE, this.X坐标, (double)(f8 + 1.0F), this.Z坐标, (int)(1.0F + this.width * 20.0F), (double)this.width, 0.0D, (double)this.width, 0.20000000298023224D, new int[0]);
                        ...
                        else
                        {
                            ...
                            double d13 = this.X坐标 + (double)(f10 * (float)this.ticksCatchableDelay * 0.1F);
                            double d15 = (double)((float)MathHelper.floor_double(this.getEntityBoundingBox().minY) + 1.0F);
                            double d16 = this.Z坐标 + (double)(f11 * (float)this.ticksCatchableDelay * 0.1F);
                            Block block1 = worldserver.getBlockState(new 阻止位置((int)d13, (int)d15 - 1, (int)d16)).getBlock();
                    ...
                    else if (this.ticksCaughtDelay > 0)
                    {
                        ...
                        if (this.rand.nextFloat() < f1)
                        {
                            ...
                            double d12 = this.X坐标 + (double)(MathHelper.sin(f9) * f2 * 0.1F);
                            double d14 = (double)((float)MathHelper.floor_double(this.getEntityBoundingBox().minY) + 1.0F);
                            double d6 = this.Z坐标 + (double)(MathHelper.cos(f9) * f2 * 0.1F);
                            Block block = worldserver.getBlockState(new 阻止位置((int)d12, (int)d14 - 1, (int)d6)).getBlock();
    ...
    public int handleHookRetraction()
    {
        ...
        else
        {
            ...
            if (this.caught实体 != null)
            {
                double d0 = this.angler.X坐标 - this.X坐标;
                double d2 = this.angler.Y坐标 - this.Y坐标;
                double d4 = this.angler.Z坐标 - this.Z坐标;
                double d6 = (double)MathHelper.sqrt_double(d0 * d0 + d2 * d2 + d4 * d4);
                double d8 = 0.1D;
                this.caught实体.通便X += d0 * d8;
                this.caught实体.motionY += d2 * d8 + (double)MathHelper.sqrt_double(d6) * 0.08D;
                this.caught实体.通便Z += d4 * d8;
                i = 3;
            }
            else if (this.ticksCatchable > 0)
            {
                实体Item entityitem = new 实体Item(this.worldObj, this.X坐标, this.Y坐标, this.Z坐标, this.getFishingResult());
                double d1 = this.angler.X坐标 - this.X坐标;
                double d3 = this.angler.Y坐标 - this.Y坐标;
                double d5 = this.angler.Z坐标 - this.Z坐标;
                double d7 = (double)MathHelper.sqrt_double(d1 * d1 + d3 * d3 + d5 * d5);
                double d9 = 0.1D;
                entityitem.通便X = d1 * d9;
                entityitem.motionY = d3 * d9 + (double)MathHelper.sqrt_double(d7) * 0.08D;
                entityitem.通便Z = d5 * d9;
                this.worldObj.spawnEntityInWorld(entityitem);
                this.angler.worldObj.spawnEntityInWorld(new 实体XPOrb(this.angler.worldObj, this.angler.X坐标, this.angler.Y坐标 + 0.5D, this.angler.Z坐标 + 0.5D, this.rand.nextInt(6) + 1));
                i = 1;
            }
