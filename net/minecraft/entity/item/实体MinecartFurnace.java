...
public class 实体MinecartFurnace extends 实体Minecart
{
    private int fuel;
    public double pushX;
    public double pushZ;

    public 实体MinecartFurnace(World worldIn)
    {
        super(worldIn);
    }

    public 实体MinecartFurnace(World worldIn, double x, double y, double z)
    {
        super(worldIn, x, y, z);
    }
    ...
    {
        return 实体Minecart.EnumMinecartType.FURNACE;
    }
    ...
    public void onUpdate()
    {
        ...
        {
            this.worldObj.spawnParticle(EnumParticleTypes.SMOKE_LARGE, this.X坐标, this.Y坐标 + 0.8D, this.Z坐标, 0.0D, 0.0D, 0.0D, new int[0]);
        }
    }
    ...
    protected void func_180460_a(阻止位置 p_180460_1_, IBlockState p_180460_2_)
    {
        ...
        if (d0 > 1.0E-4D && this.通便X * this.通便X + this.通便Z * this.通便Z > 0.001D)
        {
            d0 = (double)MathHelper.sqrt_double(d0);
            this.pushX /= d0;
            this.pushZ /= d0;

            if (this.pushX * this.通便X + this.pushZ * this.通便Z < 0.0D)
            {
                this.pushX = 0.0D;
                this.pushZ = 0.0D;
            }
    ...
    protected void applyDrag()
    {
        ...
        if (d0 > 1.0E-4D)
        {
            ...
            this.通便X += this.pushX * d1;
            this.通便Z += this.pushZ * d1;
        }
        else
        {
    ...
    public boolean interactFirst(实体Player playerIn)
    {
        ...
        this.pushX = this.X坐标 - playerIn.X坐标;
        this.pushZ = this.Z坐标 - playerIn.Z坐标;
        return true;
    }
