...
public class 实体ArmorStand extends 实体LivingBase
{
    ...
    public 实体ArmorStand(World worldIn)
    {
        super(worldIn);
    ...
    public 实体ArmorStand(World worldIn, double posX, double posY, double posZ)
    {
        this(worldIn);
    ...
    public boolean replaceItemInInventory(int inventorySlot, ItemStack itemStackIn)
    {
        ...
        if (itemStackIn != null && 实体Living.getArmorPosition(itemStackIn) != i && (i != 4 || !(itemStackIn.getItem() instanceof ItemBlock)))
        {
            return false;
        }
        else
    ...
    {
        List<实体> list = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.getEntityBoundingBox());
        ...
        if (list != null && !list.isEmpty())
        {
            for (int i = 0; i < list.size(); ++i)
            {
                ...
                if (实体 instanceof 实体Minecart && ((实体Minecart) 实体).getMinecartType() == 实体Minecart.EnumMinecartType.RIDEABLE && this.getDistanceSqToEntity(实体) <= 0.2D)
                {
    ...
    private void func_175422_a(实体Player p_175422_1_, int p_175422_2_)
    {
    ...
    public boolean attackEntityFrom(DamageSource source, float amount)
    {
        ...
        else if (!this.isEntityInvulnerable(source) && !this.canInteract && !this.hasMarker())
        {
            ...
            else
            {
                ...
                {
                    if (source.getSourceOfDamage() instanceof 实体Arrow)
                    {
                        source.getSourceOfDamage().setDead();
                    }

                    if (source.getEntity() instanceof 实体Player && !((实体Player)source.getEntity()).capabilities.allowEdit)
                    {
                        return false;
                    }
    ...
    private void playParticles()
    {
        ...
        {
            ((WorldServer)this.worldObj).spawnParticle(EnumParticleTypes.BLOCK_DUST, this.X坐标, this.Y坐标 + (double)this.height / 1.5D, this.Z坐标, 10, (double)(this.width / 4.0F), (double)(this.height / 4.0F), (double)(this.width / 4.0F), 0.05D, new int[] {Block.getStateId(Blocks.planks.getDefaultState())});
        }
    }
    ...
    {
        Block.spawnAsEntity(this.worldObj, new 阻止位置(this), new ItemStack(Items.armor_stand));
        this.dropContents();
    }
    ...
    private void dropContents()
    {
        for (int i = 0; i < this.contents.length; ++i)
        {
            if (this.contents[i] != null && this.contents[i].stackSize > 0)
            {
                if (this.contents[i] != null)
                {
                    Block.spawnAsEntity(this.worldObj, (new 阻止位置(this)).up(), this.contents[i]);
                }
