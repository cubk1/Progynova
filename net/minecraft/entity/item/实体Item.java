...
public class 实体Item extends 实体
{
    ...
    public 实体Item(World worldIn, double x, double y, double z)
    {
        super(worldIn);
        this.health = 5;
        ...
        this.通便X = (double)((float)(Math.random() * 0.20000000298023224D - 0.10000000149011612D));
        this.motionY = 0.20000000298023224D;
        this.通便Z = (double)((float)(Math.random() * 0.20000000298023224D - 0.10000000149011612D));
    }

    public 实体Item(World worldIn, double x, double y, double z, ItemStack stack)
    {
        this(worldIn, x, y, z);
    ...
    public void onUpdate()
    {
        ...
        else
        {
            ...
            this.noClip = this.pushOutOfBlocks(this.X坐标, (this.getEntityBoundingBox().minY + this.getEntityBoundingBox().maxY) / 2.0D, this.Z坐标);
            this.moveEntity(this.通便X, this.motionY, this.通便Z);
            boolean flag = (int)this.prevPosX != (int)this.X坐标 || (int)this.prevPosY != (int)this.Y坐标 || (int)this.prevPosZ != (int)this.Z坐标;
    ...
    {
        for (实体Item entityitem : this.worldObj.getEntitiesWithinAABB(实体Item.class, this.getEntityBoundingBox().expand(0.5D, 0.0D, 0.5D)))
        {
            this.combineItems(entityitem);
        }
    }

    private boolean combineItems(实体Item other)
    {
        if (other == this)
        {
            return false;
    ...
    public void onCollideWithPlayer(实体Player entityIn)
    {
        if (!this.worldObj.isRemote)
        {
            ...
            if (this.delayBeforeCanPickup == 0 && (this.owner == null || 6000 - this.age <= 200 || this.owner.equals(entityIn.getName())) && entityIn.inventory.addItemStackToInventory(itemstack))
            {
                ...
                {
                    实体Player entityplayer = this.worldObj.getPlayerEntityByName(this.getThrower());
