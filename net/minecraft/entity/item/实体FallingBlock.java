...
public class 实体FallingBlock extends 实体
{
    ...
    public 实体FallingBlock(World worldIn)
    {
        super(worldIn);
    }

    public 实体FallingBlock(World worldIn, double x, double y, double z, IBlockState fallingBlockState)
    {
        super(worldIn);
            ...
            if (!this.worldObj.isRemote)
            {
                ...
                if (this.onGround)
                {
                    ...
                    if (this.worldObj.getBlockState(blockpos1).getBlock() != Blocks.piston_extension)
                    {
                        ...
                        {
                            if (this.worldObj.canBlockBePlaced(block, blockpos1, true, EnumFacing.UP, (实体)null, (ItemStack)null) && !BlockFalling.canFallInto(this.worldObj, blockpos1.down()) && this.worldObj.setBlockState(blockpos1, this.fallTile, 3))
                            {
    ...
    public void fall(float distance, float damageMultiplier)
    {
        ...
        if (this.hurtEntities)
        {
            ...
            if (i > 0)
            {
                List<实体> list = Lists.newArrayList(this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.getEntityBoundingBox()));
                boolean flag = block == Blocks.anvil;
                DamageSource damagesource = flag ? DamageSource.anvil : DamageSource.fallingBlock;

                for (实体 实体 : list)
                {
                    实体.attackEntityFrom(damagesource, (float)Math.min(MathHelper.floor_float((float)i * this.fallHurtAmount), this.fallHurtMax));
                }
